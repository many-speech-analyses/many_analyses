...
Humans filter the world in irrational ways [e.g., @tversky1974judgment], seeing coherent patterns in randomness [@brugger2001], convincing themselves of the validity of prior expectations ["I knew it", @nickerson1998confirmation], and perceiving events as being plausible in hindsight ["I knew it all along", @fischhoff1975hindsight]. <!-- JC: If helpful we could also give a concrete common example here, like regarding forecasting the weather or something -->
...
## The data set: The acoustic properties of redundant modifiers

Our data set was collected in order to answer the following research question: Do speakers acoustically modify utterances to signal atypical word combinations (e.g. the atypical *a blue banana* vs the typical *a yellow banana*)?
More specifically, we are interested in the acoustic profile of referring expressions made up of a noun and an adjective modifier.
...
If the competitor and target objects differed but their color did not (e.g. *yellow banana* vs *yellow tomato*), the noun was in focus (the Noun Focus condition, NF).
If the the objects were the same object but differed in color (e.g. *yellow banana* vs *blue banana*), the color adjective was discourse-pragmatically focused (the Adjective Focus condition, AF).
If both the color and the object differed (e.g. *yellow banana* vs *blue tomato*), the whole noun phrase was in focus (the Adjective/Noun Focus condition, ANF).
...
The combinations were either typical (e.g. *Orangene Mandarine* 'orange mandarin'), medium typical (e.g. *Grüne Tomate* 'green tomato'), or atypical (e.g. *Gelbe Kirsche* 'yellow cherry').
...
The reviewer will then be asked in an online survey (LINK) to rate both the acoustic and the statistical analyses on several criteria, using a scale of 0-100:
<!-- TR:Need flow chart here-->

* Overall appropriateness of the acoustic analysis.
* Overall appropriateness of the statistical analysis.
* Choice of statistical analysis type.
* Model variables choice.
* Model specification.
* Data exclusion.
* Data transformation.
...
Furthermore, the reviewer will be given the option to include further comments in a text box.
...
These will be entered in the model formula with a measurement-error term (`me(mean, sd)` in brms).
...
- *Research experience* [numeric] as the elapsed time from PhD award. Negative values will indicate that the person is a student or graduate studens ($rexp_i$).
