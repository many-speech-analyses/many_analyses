...
Analytical flexibility has been widely discussed from a conceptual point of view [@simmons2011false; @wagenmakers2012agenda; @nosek2014method] and in regard to its application in individual scientific fields [e.g. @wicherts2016, Charles et al. 2019, @roettger2019researcher], notwithstanding, there are still many unknowns regarding the extent of analytical plurality in practice.
...
When looking at phrase-level temporal domains, the number of possible analytical pipelines increases exponentially.
...
As a second step, we will investigate (a) the extent to which the individual standardized effect sizes are affected by a series of predictors related to analytical and demographic factors (see below); and (b) the extent to which deviations from the meta-analytic estimate by the individual standardized effect sizes relate to those same predictors.
...
**Models specification**. The models for (a) and (b) from above will be fitted as measurement-error models, both with the same predictors as detailed in the preceding paragraphs.
The outcome variable of model (a) will be the standardized effect sizes, while that of model (b) the deviation of these from the meta-analytical effect size.
The deviation of the standardized effects will be calculated as the difference of the standardized effect sizes from the meta-analytical point estimate.
The measure of uncertainty of the effect deviation will be calculated as the joint standard deviation by taking the square root of the joint variance $Var(\eta_i) + Var(\hat{\eta}) + 2Cov(\eta_i, \hat{\eta})$.
