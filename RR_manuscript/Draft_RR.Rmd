...
(1) "I hope Liz does not go to the grocery store tomorrow".
...
Given the continuous and transient nature of speech, identifying  speech parameters and temporal domain becomes a non-trivial task.
...
For example, when comparing two realization of example (1) (here repeated as 2), one of which is intended to signal emphasis on *tomorrow* and one of which emphasizes *grocery store*. 

(2a) "I hope Liz does not go to the grocery store TOMORROW".
(2b) "I hope Liz does not go to the GROCERY STORE tomorrow"

Do we only compare the word *tomorrow* in 2a and 2b or also the word *grocery store* or do we measure utterance wide acoustic profiles? Do we measure the whole word? Or just the stressed syllable? 
Do we measure average the domain or do we measure a specific point in time? Do we measure fundamental frequency or intensity? When looking at phrase-wide temporal domains, the number of possible analytical pipelines quickly explodes (see Figure x)

<!-- TR: How about a plot like Figure 2 in my RDF paper: https://www.journal-labphon.org/articles/10.5334/labphon.147/ -->
...
<!-- TODO: include citation to ECO RR -->
...
<!-- TODO: ADD FORMULA of linear regression model -->
FORMULA
...
<!-- SC: it would be good to decide on a clear terminology to refer to the individual original models, the individual models that we run (to refit the original models), and the meta-analytical model.  -->
...
<!-- TODO: DESCRIBE HOW WE WILL LOOK AT demographics -->
