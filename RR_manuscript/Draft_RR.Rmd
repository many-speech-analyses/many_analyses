...
# Function for reporting posteriors
...
Specifically, we discuss the degree of variability in research results obtained by 30 teams who had to choose the operationalisation and statistical procedures to answer the same research question, on the basis of the same set of raw data (here, speech recordings).
...
For the present study, 30 analysis teams have received access to the entire data set generated by the production study.
...
We will employed a meta-analytic approach to assess (i) the variability of the reported effects, and (ii) how analytic and researcher-related predictors affect the final results.

In this study, we followed the procedures proposed by @Parker2020 and @aczel2021.
The project involved the following five phases:
...
We have initially estimated that this process, from the time of an in-principle acceptance of the Stage 1 Registered Report to the end of Phase 5, would have taken nine months.
...
Phase 4. (meta-analysis) took longer than initially thought and the total duration of the project ended up lasting 12 months.
...
All individuals interested in participating were asked to complete a questionnaire detailing their familiarity with numerous analytical approaches common in the speech sciences (`analytic_approach_quest.pdf`).
...
We received initial expression of interest to participate from more than 200 analysts, although there has been a substantial drop-out rate (see Section Results).
...
We then requested that they answer the question: *Do speakers acoustically modify utterances to signal atypical word combinations?*
To do so, they were given the data generated by the experiment described in Section [The data set](#s:dataset).
The data included the audio recordings with corresponding time-aligned transcriptions in the form of Praat TextGrid files.
...
Each analysis was evaluated by four different analysts teams who functioned as peer-reviewers.
...
The sequences was systematically assigned so that, if possible, each analysis is allocated to each position in the sequence for at least one reviewer.
...
This included the narrative methods section, the analysis team's answers to the questionnaire regarding their methods, including analysis code and the data set.
...
After submitting the review, a methods section from a second analysis was then made available to the reviewer.
This same sequence was followed until all analyses allocated to a given reviewer were provided and reviewed.^[We initially planned to simultaneously present all four (or more) methods sections to each reviewer after the fourth round, with the option to revise their original ratings and provide an explanation. We have decided to skip this last step due to time constrains.]
...
These summary statistics are reported in \@ref{descr-stats} of the Results section.
...
As the measure of variability, we took the meta-analytic \textsc{group-level standard deviation} ($\sigma_{\alpha_{\text{t}}}$, see below), where the groups are the analysis teams.
As we detail in Section \@ref{meta-est}, we have also run further non-preregistered analyses.
...
<!-- TODO: add link to refitting procedure -->
Details about the refitting procedure can be found at LINK.
...
After having obtained the standardized effect sizes $\eta_i$ with related standard errors $\text{se}_i$, for each critical predictor of the individual reported model, we conducted a \textsc{Bayesian random-effects meta-analysis} using a multi-level (intercept-only) regression model.
...
We fitted this model with 4 chains of Hamiltonian Monte-Carlo sampling for the estimation of the joint posterior distribution, using the No U-Turn Sampler (NUTS) as implemented in Stan [@stan2021], and 4000 iterations (2000 for warm-up) per chain, distributed across 8 processing cores and 2 threads in within-chain parallelization.
...
The model did not incur in divergent transitions ($\hat{R}$ was equal or lower than 1) and the estimated sample sizes were sufficient.
<!-- TODO: add link to meta-model code -->
The code used to run the model can be found at <>.
...
For each pair of analyses $(X, Y)$, the SDI was obtained using the following formula:
...
The major measurement dimension of each analysis was categorized according to the following possible groups: *duration*, *intensity*, *fundamental frequency* (f0), *other spectral properties* (e.g. frequency, center of gravity, harmonics difference, etc.), and *other measures* (e.g. principal components, vowel dispersion, etc.).
...
We realised when fitting the model that the measure of conservativeness is related to the standard error of the estimates (i.e. more group-level effects = higher standard error).
There was no team that declared to have made post-hoc changes to the analyses.
...
The outcome variable of the model was the standardized effect sizes and related standard deviation.
...
As the prior for the intercept and the predictors we used a normal distribution with mean 0 and standard deviation 1.
The model was run with the same settings as with the meta-analytic model.
<!-- TODO: add link to predictor model code -->
The code used to run the model can be found at <LINK>.
...
We have exclude from our synthesis any individual analysis submitted after we have completed peer review or those unaccompanied by analysis files that allow us to understand what the analysts did.
We also excluded any individual analysis that does not produce an outcome that can be interpreted as an answer to our primary question.
<!-- TODO: mention that there were some and add info in descriptive stats section? -->
...
Analysts had on average `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", avg)` years (SD = `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", sd)`) of experience after completing their PhD, ranging from `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", Min.)` years, i.e. PhD students (or younger) to `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", Max)` years, suggesting that, on average, analyst were experienced researchers.
...
We take this to suggest that analysts had an overall rather high positive prior belief in the investigated relationship between acoustics and word combination typicality.
...
`r round(random_n[[3,1]],2) * 100`\% of linear models specified no random effects at all (without pooling their data), so effectively ignoring these non-independences [@hurlbert1984pseudoreplication].
...
When considering both acoustic and statistical analysis, we have found a grand total of `r nrow(unique_acou_stat)` different analytic pipelines.
In other words, each individual submitted analysis was unique.
...
## Meta-analytic estimation {#meta-est}
...
Estimates ranged between `r round(min(msa_models$estimate),2)` to `r round(max(msa_models$estimate),2)` standard units. 
...
Neither analysts' prior beliefs in the phenomenon `r report_posterior(predictors_bm_rintercepts, "prior_belief_s")`, nor their seniority in terms of years after completing their PhD `r report_posterior(predictors_bm_rintercepts, "years_from_phd_s")` seem to compellingly affect model estimates.
...
Interestingly, the model uniqueness, i.e. how unique the choice and combination of predictors is, affects the analysts estimate, with more unique models producing higher positive estimates `r report_posterior(predictors_bm_rintercepts, "pop_sdi_s")`. 
...
In other words, depending on the choice of measurement and the measurement window, analysts arrived at opposite conclusions about how and if typicality is expressed acoustically. 
...
The reported effect estimates from teams varied between `r round(min(msa_models$estimate),2)` and `r round(max(msa_models$estimate),2)` standard units which corresponds to duration values ranging from `r round(min(msa_models$estimate) * 33.4,2) ` to `r round(max(msa_models$estimate) * 51.4,2)`.
...
For example, the phenomenon of incomplete neutralization, a widely studied phenomenon, involves duration effects ranging from 7 to 15 ms [@nicenboim2018using]. This particular phenomenon has sparked long-lasting methodological and theoretical debates about the very nature of linguistic representations [@port2005against].
Vowel duration differences within this range have been reported across phenomena, associated with segmental contrasts [@coretta2019], reduction [@nowak2006], and biomechanical reflexes of prominence [@mucke2014effect]. 
Expecting random variation between different analysts of `r round(3.8*1.96)` to `r round(7.7*1.96)` in one or the other direction can be theoretically relevant and might lead to opposing theoretical conclusions.
...
Second and orthogonal to a possible true relationship, certain measurement choices might be associated with stronger expectations relative to the research question, which might lead to stronger researcher biases.
Many analysts targeted measures related to fundamental frequency (f0), likely because similar functional relationships like information structure and predictability can be expressed by f0 [e.g. @grice2017integrating; @turnbull2017role]. 
...
Participating analysts might have been aware of those findings, which might have, subconsciously or otherwise, nudged their choices into one particular direction.
...
One team coordinator gave the following reason for dropping out of the project (and allowed us to share this statement): "[T]he project's approach is too much top-down instead of bottom-up. [...] I would rather prefer to explore the collected utterances and run different types of acoustic analyses, which would then be followed by statistical evaluation of the data."
We find that this statement speaks to the main objective of the current study: investigate researcher's degrees of freedom in the speech sciences.
...
Second, making analytic pathways completely re-traceable does not change the fact that analysts apply idiosyncratic analytic approaches.
...
We invite other researchers to re-analyse our data and try to replicate our results using a different research question.
...
Despite of all these possible sources of bias and uncertainty, our study suggests that at least the minimum variability that we can expect to find across researchers is the rather concerning variability that we have observed.

Finally, our study focused on the speech sciences.
...
This is an important point to make because the cognitive sciences in general, and the language sciences in particular have many research areas that are based on qualitative methods.
