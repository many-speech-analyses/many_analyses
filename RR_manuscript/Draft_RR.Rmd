...
All models converged ($\hat{R}$ was approximately 1) and only some had a small number of divergent transitions which were not deemed problematic (range XXX).
...
Note that due to the unforeseen variability across teams, the latter exclusion criteria were not preregistered and were applied after sighting all analyses strategies. 
...
This means that, in addition to the critical predictor (typicality of the adjective noun combinations), models had on average one additional predictor (range = `r round(predictor_n$min_predictor,1)- round(predictor_n$max_predictor,1)`).
...
Of the 170 critical model coefficients, `r n_effects` were claimed to show a statistically reliable effect (`r round(perc_effects,1)`%). 
