...
In conjunction with an academic incentive system that rewards certain discovery processes more than others [@sterling1959publication; @koole2012rewarding], we often find ourselves exploring many possible analytical pipelines, but only reporting a select few.
<!-- NOTE: (JVC) in connection with = referring to; in conjunction with = together, coupled with. I think we want the later, but change if I'm mistaken -->
...
Analytical flexibility has been widely discussed from a conceptual point of view [@simmons2011false; @wagenmakers2012agenda; @nosek2014method] and in regard to its application in individual scientific fields [e.g. Wichert et al. 2015, Charles et al. 2019, @roettger2019researcher], notwithstanding, there are still many unknowns regarding the extent of analytical plurality in practice.
<!-- TODO: add refs for wichert et al. 2015, Charles et al. 2019... are we certain this isn't @wicherts2016? (JVC) -->
...
The acoustic parameters of one category are usually asynchronous, i.e. they appear at different time points in the unfolding signal, and overlap with parameters of other categories [e.g. @jongman2000acoustic; @lisker1986voicing; @summerfield1984; @winter2014spoken].
...
When looking at phrase-level temporal domains, the number of possible analytical pipelines increases exponentially. <!-- NOTE: (JVC) this is hyperbole, but seems (possibly) more appropriate than "explodes" -->
This plurality of analytical paths is illustrated in Figure \@ref(fig:forkingPaths).
<!-- When comparing two utterances such as (2a) and (2b), there are many things to consider.  NOTE: (JVC) (I think we can do without this sentence.) -->
...
<!-- NOTE: (TR) is the figure better now? -->
Figure \@ref(fig:forkingPaths), illustrates a specific option to automatically calculate f0.
Knowing that the signals and the measures thereof can be noisy, another option would be to smooth them using one algorithm and degree over others, or remove values that substantially deviate from surrounding values or expectations, either manually or automatically, and so on. <!-- NOTE: (JVC) Im having a bit of trouble with this sentence. I tried to fix it and now I am realizing I don't quite follow (JVC) -->
...
To assess the variability in data analysis pipelines across independent researchers, we will provide analytical teams with an experimentally elicited speech production data set.
...
Moreover, the data is not available, and the statistical analysis used anti-conservative co-variance structures, leaving reason to doubt the original conclusions. <!-- NOTE: (JVC) "lending doubt on" sounded a bit weird to me (JVC) -->
...
<!-- TODO: see XXX, what do we refer to here? Are we going to include an image? -->
...
<!-- QUESTION: wanna meditate over the term demographic (add analytic vs. analytical too)-->
...
<!-- QUESTION: How to frame this sentence. It is not ongoing. Alternatively: "Recruitment for this project will commence as soon as the present Registered Report is in-principle accepted. (This seems good to me. added) -->
...
Following Parker et al. \highlight{[ECO RR]}, we identified a minimal sample size that would provide an estimate of heterogeneity of the effect sizes reported by the analysis teams ($\tau^2$, i.e. the variance $\sigma^2_{\alpha_{\text{t}}}$, see \Cref{s:meta-est}).
...
The probability $p$ that a CrI interval does not include 0 is obtained via the *t*-statistic:
...
Given $p$ and $\tau^2$, we can calculate $n$ such that $p < 0.05$.
Inserting $SE(\tau^2)$ into the formula for the *t*-statistic shows that, when $n$ is fixed, *t* (and hence $p$) will be the same regardless of $\tau^2$:
...
Once their analysis is complete, they will answer a structured survey (LINK), providing information about their analysis technique, an explanation of their analytical choices, their quantitative results, and a statement describing their conclusions.
...
<!-- NOTE: TR: don't know what you mean tbh -->
<!-- NOTE: JVC: I get what you're saying, essentially we will keep the levels of their categorical variables the same when we refit the models, right? I changed it so that it says that. Change if wrong. -->
...
<!-- NOTE: I understand "posterior probability" to refer to a point estimate, i.e., the posterior probability that B > 0 = some value between 0 and 1. I may be mistaken, but I think you are referring to the posterior distribution in the above paragraph -->
...
<!-- NOTE: I keep changing meta-analytical to meta-analytic (the -cal sounds weird to me). My unscientific google searching shows that people use both, but meta-analytic is much more common (nearly 4 million hits vs. 500k) -->
<!-- NOTE: (SC) I tend to use analytical to mean "related to analysis" and "analytic" in opposition to "synthetic" (from their use in logic and linguistics) but I am flex as long as we are consistent. -->
...
As a second step, we will investigate (a) the extent to which the individual standardized effect sizes are affected by a series of predictors related to analytical and demographic factors (see below); and (b) the extent to which deviations from the meta-analytic point estimate by the individual standardized effect sizes relate to those same predictors.
...
The coefficient $\alpha_{\text{t}[i]}$ can be interpreted as the deviation score of the team's $\eta_i$ from the meta-analytic point estimate, allowing us to determine potential effects of analytical and demographic factors on how the teams deviate from the meta-analytic mean.
Since each $\alpha_{\text{t}[i]}$ comes with uncertainty, quantified by its standard deviation $\sigma_{\alpha_{\text{t}[i]}}$, we will fit a measurement-error model, as we did in the meta-analytic model above.
