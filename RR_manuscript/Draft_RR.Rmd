...
In this article, we investigate the impact of analytic choices on research results when many analyst teams examine the same speech production data set, a process that involves both decisions regarding the *operationalization* of a complex signal and decisions regarding *statistical analysis*.
...
Consequently, a good part of published papers likely present overconfident interpretations of data and statistical results based on idiosyncratic analytic strategies [e.g., @simmons2011false; @gelman2014statistical].
...
Again, they found vast heterogeneity in researchers' conclusions which was not predicted by the researchers' expertise, but rather seem to be more dependent on the research question itself.
...
We received initial expression of interest to participate from more than 200 analysts, though there was a substantial drop-out rate (see Section Results).
...
The model did not incur any divergent transitions ($\hat{R}$ was equal or lower than 1) and the estimated sample sizes were sufficient.
...
We excluded from our synthesis any individual analysis submitted after peer review (Phase 3) or those unaccompanied by analysis files without which it was not possible to understand what the analysts did.
...
The research compendium of the study containing all the code and data presented here and can be found at <https://osf.io/3bmcp/>.
...
Of the submitted analyses, the initiating authors identified `r nrow(msa_models)` from `r pull_from_tib(team_desc_df, stage, "Final", n_teams)` teams that were deemed eligible to be included in the meta-analytic synthesis. 
In what follows, we describe the characteristics of these teams and analyses. 
...
At the end of Phase 2 (primary data analysis), the teams had submitted a grand total `r no_analyses` individual models to answer the research question, with a mean of `r pull_from_tib(tbl_model_desc, label, "model_desc_models", avg)` (SD: `r pull_from_tib(tbl_model_desc, label, "model_desc_models", sd)`) models per team. 
...
`r (round(outcome_n[[1,1]],2) * 100) %>% xfun::numbers_to_words() %>% str_to_sentence()` percent of the models used a measure of duration as the outcome variable, `r round(outcome_n[[2,1]],2) * 100`\% used fundamental frequency (f0), `r round(outcome_n[[3,1]],2) * 100`\% used vowel formants, `r round(outcome_n[[4,1]],2) * 100`\% intensity, and `r round(outcome_n[[5,1]],2) * 100`\% other measures.
...
`r (round(random_n[[3,1]],2) * 100) %>% xfun::numbers_to_words() %>% str_to_sentence()` percent of the linear models specified no random effects at all (without pooling their data), so effectively ignoring these non-independences [@hurlbert1984pseudoreplication].
...
This means that the mean deviations of any individual team from the meta-analytic effect estimate range between `r pm(round((meta_bm_prereg_tidy$conf.low[['sd_animal__Intercept']] * 1.96), 2))` to `r pm(round((meta_bm_prereg_tidy$conf.high[["sd_animal__Intercept"]] * 1.96), 2))` (0.3 * 1.96, 0.07 * 1.96) in standard units at 95% probability.
...
The standard solution to this problem is to apply a correction for family-wise error (i.e., alpha inflation), which consequently reduces power. 
...
Being aware of this idiosyncratic variation between analysts should lead to more nuanced claims and what @breznau2021observing refer to as "epistemic humility" (p. 10).
...
This uncertainty is possibly further inflated by the fact that the research question presented to the teams was vague, despite being of a kind normally found in the speech science literature: *Do speakers acoustically modify utterances to signal atypical word combinations?* 
...
We focused on quantitative analyses that require the operationalization of a multidimensional signal. 
...
Importantly however, we found some evidence for systematic effects on team's estimates based on what and how they measured the speech signal, biasing their estimates in one direction or the other. 
