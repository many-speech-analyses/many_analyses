...
<!-- TODO: Add link to the collaborative document maybe? -->
...
Of the `r n_effects` submitted analyses, the initiating authors identified `r nrow(msa_models)` critical model coefficients from `r pull_from_tib(team_desc_df, stage, "Final", n_teams)` teams that were deemed eligible to be included in the meta-analytic synthesis. 
...
At the end of Phase 2 (primary data analysis), the teams had submitted a grand total of `r no_analyses` individual models (including `r nrow(msa_models)` critical model coefficients) to answer the research question, with a mean of `r pull_from_tib(tbl_model_desc, label, "model_desc_models", avg)` (SD: `r pull_from_tib(tbl_model_desc, label, "model_desc_models", sd)`) models per team. 
...
<!-- It looks like this is on the level of critical coefficients = 170, rather than submitted models, double check --> 
...
<!-- TR: remove reference to meta-analytical estimate -->
...
We will interpret these standard units relative to phonetic measures in the discussion section, but to provide an intuition about the absolute scale of variation, we can express them on a standardized scale that does not require domain expertise: The IQ scale [@kamphaus2019clinical] which is defined by a standard deviation of 15. 
The observed variability of estimates between teams corresponds to variability in IQ measures up to 14.9 IQ points.
...
```{r plot-meta1, echo = FALSE, fig.cap="Standardized effect sizes across all critical coefficients provided by the teams. Raw estimates are displayed in grey. Estimates after shrinkage as provided by the meta-analytic model are displayed in black/orange.", out.width="100%"}
...
Being aware of this idiosyncratic variation between analysts should lead to more nuanced claims and a certain level of epistemic humility.
