...
@dutilh2019 ad @starns2019assessing investigated analysts' choices when inferring theoretical constructs based on the same data set using computational models.
...
For instance, if the speaker is exhausted by all their meetings, they might acoustically highlight the word *another* or *meeting* to contrast it with other, more pleasant activities.
...
The unpublished study we took the data from aimed at investigating whether speakers acoustically modify utterances to signal unexpected referring expressions.
In the following section we introduce the research question and the experimental procedure of said study, and we describe the dataset as it was given to the analysts.
...
This account, however, has mainly focused on content selection [@gatt2013we], i.e. whether a certain referential expression is chosen or not, while not taking into consideration the indexical potential of speech.
...
If the competitor and target objects differ but their color did not (e.g. *yellow banana* vs *yellow tomato*), the narrow focus is on the noun (the Noun Focus condition, NF).
If the objects are the same but differ in color (e.g. *yellow banana* vs *blue banana*), the narrow focus is on the color adjective (the Adjective Focus condition, AF).
If both the color and the object differ (e.g. *yellow banana* vs *blue tomato*), the narrow focus is on the whole noun phrase (the Adjective/Noun Focus condition, ANF).
...
For the present study, N analysis teams were given access to the entire data set generated by the production study and were instructed to answer the following research question: *Do speakers acoustically modify utterances to signal atypical word combinations in referring expressions?*
...
...
We are closely following the data collection procedure proposed by @Parker2020.
...
The initiating authors (SC, JC, TR) created an online landing page providing a general description of the project and a short pre-recorded slide show that summarizes the study and research question (<https://many-speech-analyses.github.io>).
...
For the sake of simplicity, we will refer to single researcher or small teams as \textsc{analysis teams}.
Recruitment for this project will commence upon receiving in-principle acceptance.
As outlined above, our aim is to assess the variability of the reported effects and how these are affected by a set of predictors, rather than the meta-analytical estimate *per se*.
...
The analysis teams will register and answer a demographic and expertise questionnaire (`intake_form.pdf`).
PDF versions of all the questionnaires used in this study are available at LINK^[Temporary link: <https://osf.io/h6z8w/?view_only=3457f2b01a5f4435b3d917eb7b5134e8>.].
...
The data will include the audio recordings with corresponding time-aligned transcriptions in the form of Praat TextGrid files (LINK^[Temporary link: <https://osf.io/5agn9/?view_only=3429acfd8eae4f0aafe2e17ad5b0983f>]).
...
As the measure of variability, we will take the meta-analytic \textsc{group-level standard deviation} ($\sigma_{\alpha_{\text{t}}}$, see below), where group refers to the analytical teams.
...
Moreover, to account for the differing degree of uncertainty around each standardized effect size, we will use the standard deviation of each effect size returned by the standardized models as the \textsc{standardized standard error} ($\text{se}_i$).
...
After having obtained the standardized effect sizes $\eta_i$ with related standard errors $\text{se}_i$, for each critical predictor of the individual reported analyses, we will fit a \textsc{Bayesian random-effects meta-analysis} using a multilevel (intercept-only) regression model.
...
<!-- TODO: add LINK to code of the meta-analytic model: add to landing page in non-linked page -->
...
<!-- TODO: add LINK to code of the predictors model: add to landing page in non-linked page -->
The code used to run the model can be found here: INSERT LINK.
...
- **Standardized effect sizes**: ($\eta_i$) effect sizes returned by the standardised models.
...
- **Meta-analytic group-level standard deviation**: ($\sigma_{\alpha_{\text{t}}}$) group-level standard deviation returned by the meta-analytic model.
