...
As explained in the **Methods** section, the primary aim of this analysis is to assess the degree of between-team variability.
...
<!-- TR: This is all really confusing. I know what you mean only because we talked about it.-->
...
Given that the estimated between-model variability in standard units as we obtained from the meta-analytic estimate is between `r round(meta_bm_tidy$conf.low[["sd_model_id__Intercept"]], 2)` and `r round(meta_bm_tidy$conf.high[["sd_model_id__Intercept"]], 2)`, we can convert the standard units into milliseconds by multiplying them with the standard deviation values in milliseconds: $33.4 * 0.11 = 3.8$ ms, $51.4 * 0.11 = 5.7$ ms;  $33.4 * 0.15 = 5$ ms, $51.4 * 0.15 = 7.7$ ms.
We can thus calculate 95% intervals of the estimated between-model variability in milliseconds: `r round(3.8*1.96, 1)`-`r round(5.7*1.96, 1)` ms based on the low CI estimate 0.11, `r round(5*1.96, 1)`-`r round(7.7*1.96, 1)` ms based on the high CI estimate 0.15.

<!-- TR: Sorry but this is unreadable. I will completely overhaul it.-->
...
We gave `r team_desc_list$Final$n_teams$val` analyst teams the same speech dataset to answer the same research question: *Do speakers acoustically modify utterances to signal atypical word combinations?*.
