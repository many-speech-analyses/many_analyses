...
`r (team_desc_list$Initial$n_teams$val - team_desc_list$Final$n_teams$val) %>% xfun::numbers_to_words() %>% stringr::str_to_title()` of the signed-up teams dropped out during the analysis phase.
...
For example, if we consider those analyses who targetfundamental frequency (f0), we find that it is operationalized in a myriad of ways including the minimum, the maximum, the mean, the median, as a range in an interval or a ratio between two intervals. 
...
In light of these possible sources of bias, a field can benefit from explicit positionality statement [e.g., @fox2021open]
