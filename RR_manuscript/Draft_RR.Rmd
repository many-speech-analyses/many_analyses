...
This contrast is manifested by many acoustic features which can differ depending on several factors, such as position of the consonant in the word and context of surrounding sounds [@lisker1977rapid].
...
Concretely, a focus contrast marks one or more elements in a sentence as prominent, by different linguistic means depending on the language.
...
Each analysis was evaluated by four different analysis teams who functioned as peer-reviewers.
...
Analysts had on average `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", avg)` years (SD = `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", sd)`) of experience after completing their PhD, ranging from `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", Min.)` years, i.e. PhD students (or less experienced) to `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", Max)` years, suggesting that, on average, analyst were experienced researchers.
...
**Non-preregistered**. However, we did not take into account in our preregistration that teams might submit multiple analyses/models which, if unaccounted for, violates the independence assumption. 
...
In order to answer this question, teams had to operationalize parameters of a multidimensional signal, operationalize and choose appropriate model predictors, and construct appropriate statistical models to answer the research question.
...
for example, the widely studied phenomenon of incomplete neutralization involves duration effects ranging from 7 to 15 ms [@nicenboim2018using]. This particular phenomenon has sparked long-lasting methodological and theoretical debates about the very nature of linguistic representations [@port2005against].
...
Expecting random variation between different analysts of `r round(3.8*1.96)` to `r round(7.7*1.96)` ms in one or the other direction can be theoretically relevant and might lead to opposing theoretical conclusions.
...
While properties of the teams did not seem to systematically affect results, teams' estimates seem to highly depend on certain measurement choices. 
...
Eventually, a mature scientific discipline needs to develop a set of detailed quantitative hypotheses of how conceptual constructs manifest themselves in the measured system, i.e. in the present case how communicative pressures of functions are expressed in the acoustic signal; which can be achieved, for example, by mathematically formalizing verbal expectations or using computational models [e.g., @van2020formalizing; @guest2021computational; @scheel2021hypothesis; @devezer2021case].
...
Despite all these possible sources of bias and uncertainty, our study suggests that at least the minimum variability that we can expect to find across researchers is the rather concerning variability that we have observed.

Finally, our study focused on the speech sciences.
The generalizability of our findings to other disciplines, and to other sub-disciplines of the language sciences specifically, is, of course, limited.
...
When many different analysts have to analyze the same data set to answer the same research question, analysts differ drastically in how they approach this task, leading to vastly different answers. 
...
Importantly however, we found some evidence for systematic effects on teams' estimates based on what and how they measured the speech signal, biasing their estimates in one direction or the other. 
This result, taken together with the meaningful between-model variation and the tendency to test the same research question on multiple outcome variables, suggests that a vast plurality of acceptable approaches is expected to frequently lead to inconclusive results.
