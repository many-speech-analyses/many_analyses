...
At the end of Phase 2 (primary data analysis), the teams have submitted a grand total `r no_analyses` individual models to answer the research question, with a mean of `r pull_from_tib(tbl_team_desc, label, "model_desc_n_models", avg)` (SD: `r pull_from_tib(tbl_team_desc, label, "model_desc_n_models", sd)`) models per team. 
Table \@ref(tab:team-descriptives-table) provides a summary of the contributing teams and their analyses. 
...
Table \@ref(tab:categorical-descriptives-table) provides a summary of the acoustic analyses. 
...
Table \@ref(tab:stats-descriptives-table) provides a summary of the statistical analyses. 
...
Of the submitted analyses, `r n_effects` claimed to have found a statistically reliable effect (`r perc_effects`%). 
`r n_atleast_1 %>% xfun::numbers_to_words() %>% str_to_sentence()` teams out of the `r length(unique(msa_models$animal)) %>% xfun::numbers_to_words()` (`r perc_atleast_1`%) submitting analyses reported at least one found effect. 
...
The mean rating of the acoustic analyses, on a scale from 0 to 100, is `r round(review_mean[[1]],1)` (SD = `r round(review_mean[[2]],1)`, see Table \@ref(tab:team-descriptives-table)).
...
msa_sd_sd <- bind_rows(msa_sd_sd_lo, msa_sd_sd_hi) %>%
  ...
  unite("low", low_sd_min, low_sd_max, sep = "-") %>%
  unite("high", high_sd_min, high_sd_max, sep = "-") %>%
  mutate(
...
The nested model yields a posterior 95% CrI for between-team variability of `r round(meta_bm_nest_tidy$conf.low[["sd_animal__Intercept"]], 2)` to `r round(meta_bm_nest_tidy$conf.high[["sd_animal__Intercept"]], 2)` standard units ($\beta$ = 0.02, SD = 0.01), corresponding to a mean deviation range of ±0.1 standard units.
The posterior 95% CrI for between-analysis variability (nested in teams) is `r round(meta_bm_nest_tidy$conf.low[["sd_animal:model_id__Intercept"]], 2)` to `r round(meta_bm_nest_tidy$conf.high[["sd_animal:model_id__Intercept"]], 2)` standard units ($\beta$ = 0.132, SD = 0.01).
...
**Non-preregistered**. As mentioned in the previous section, we have run an additional model, using team and model ID nested within team as random effects.
In this non-preregistered model, the meta-analytic intercept estimate is between `r round(meta_intercept_nest[3],3)` and `r round(meta_intercept_nest[4],3)` standard units (95\% CrI, mean = `r round(meta_intercept_nest[1],3)`).
...
(ref:meta_plot1) illustrates the individual intercepts for critical typicality coefficients across models and teams, sorted in ascending order based on their mean.
...
(ref:meta_plot1), panel C, displays the coefficients for all predictors alongside their 80% and 95% credible intervals. 
...
(ref:meta_plot1) displays the posterior estimates for the measurement outcome (i.e. what acoustic dimension was measured, panel A) and measurement window (i.e. what is the unit over which the outcome was measured, panel B). 
...
Analyses exhibited at least `r nrow(unique_model_specs)` unique ways of operationalizing the acoustic signal alongside `r nrow(unique_acou_oper)` unique ways of constructing their statistical model, amounting to `r nrow(unique_model_specs) * nrow(unique_acou_oper)` possible unique combinations. 
...
Consider them an indication of how much said outcome measures generally vary after being extracted from the signal (see xx for an overview <!-- TODO: add link to a vignette-->). 
...
If we are more conservative and only convert the meta-analytic estimates of between-analysis variation, we obtain an estimate of between-team variability in milliseconds that ranges from `r round(3.8*1.96, 1)` to `r round(7.7*1.96, 1)` (at 95% credibility).^[minimum SD of duration measures multiplied by lower 95% CrI of meta analysis: $33.4 * 0.11 = 3.8$ ms; maximum SD of duration measures multiplied by upper 95% CrI of of meta analysis: $51.4 * 0.15 = 7.7$ ms]. 
...
For example, XXms is considered the just noticeable difference, i.e. the threshold under which human are claimed not to be able to distinguish them {REF}. <!-- TODO: Stefano add ref here and values from your work-->
<!-- TODO: Add a second possible phenomenon from articulation-->
Another example is the phenomenon of incomplete neutralization, a widely studies phenomenon, which surrounds duration effects ranging from 7-15ms [@nicenboim2018using]. 
...
Expecting random variation between different analysts of `r round(3.8*1.96)` to `r round(7.7*1.96)` in one or the other direction is thus concerning and might lead to theoretically completely opposing interpretations.

```{r}
#msa_sd_sd
```

<!-- TODO (SC): Add table with different outcomes, means (?) and SDs plus estimated deviations based on ±0.22/0.33 SDs. -->

```{r}
#knitr::kable(msa_sd, digits = 1)
```
...
Following @breznau2021observing, we are bound to conclude that "[...] we are left to believe that idiosyncratic uncertainty is a fundamental feature of the scientific process that is not easily explained by typically observed researcher characteristics or analytic decisions."[p.9].
...
Many researchers targeted measures related to fundamental frequency (f0), since similar functional relationships like information structure and predictability can be expressed by f0 [e.g. @grice2017integrating; @turnbull2017role]. 
Moreover, prior work as actually suggested a relationship between typicality and f0 [e.g. @dimitrova2008prosodic; @dimitrova2009did]. 
...
When operating within the frequentist inferential framework, testing the same hypothesis with different dependent variables, increases the Type-I error rate if not corrected for [@tukey1953].
<!-- TR: couldnt find bibtex of tukey: Tukey, J. W. (1953). The problem of multiple comparisons. Multiple comparisons. 
JVC: I found it as an unpublished report and added tukey1953 to the refs file. Not sure if that is better than [Tukey 1953, as cited in @benjamini1995controlling] or not.-->
...
Beyond a replication crisis, the quantitative sciences are in an inference crisis [@rotello2015more; @starns2019assessing].
...
JVC: added @mcelreath2020 to bib file. -->
...
<!-- TR: This preceding section rocks IMO -->
...
Within fields that deal with speech data, open source software that allows to extract acoustic parameters via reproducible scripts can help other researchers to trace back seemingly inconsequential choices during the measurement process [e.g., Praat: @boersma2021praat, EMU: @winkelmann2017emu].

<!-- TODO: MFA, Formant tracking etc -->
...
This can be achieved by mathematically formalizing verbal expectations  or using computational models [e.g., @van2020formalizing; @guest2021computational; @scheel2021hypothesis; @devezer2021case].
...
In light of these possible sources of bias, a field can benefit from explicit positionality statements [e.g., @fox2021open]
