...
tbl_team_desc <- msa_models %>% 
  ...
  na.omit() %>% 
  group_by(metric) %>% 
  summarize(min = min(vals), max = max(vals), avg = mean(vals), sd = sd(vals)) %>% 
  mutate_if(is.numeric, round, digits = 2) %>% 
  mutate(desc = "team_desc", 
   ...
   `Mean (SD)` = glue::glue("{avg} ({sd})"), 
    Metric = case_when(
      ...
      metric == "stat_rating" ~ "Statistical ability", 
      metric == "prior_belief" ~ "Prior belief of $H_{1}$", 
      metric == "phon_rating" ~ "Phon rating", 
      metric == "all_rating" ~ "all rating"), 
    label = glue::glue("{desc}_{metric}")) %>% 
  select(label, desc, Metric, Min. = min, Max = max, `Mean (SD)`, avg, sd) %>% 
  add_row(label = " ", desc = "team_desc", Metric = " ", 
          Min. = NA, Max = NA, `Mean (SD)` = NA, avg = NA, sd = NA) %>% 
...
tbl_team_model_desc <- bind_rows(tbl_team_desc, tbl_model_desc) %>% 
  mutate(desc = case_when(
    Metric != "all rating" & desc == "team_desc" ~ " ", 
...
tbl_team_model_desc %>% 
  select(Descriptives = desc, Metric, Min., Max, `Mean (SD)`) %>% 
  knitr::kable(format = "latex", booktabs = T, escape = F, 
    linesep = "", align = c("l", "l", "r", "r", "r"))
