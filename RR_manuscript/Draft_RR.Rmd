...
Beyond the difficulties of replicating scientific findings, a growing body of evidence suggests that the theoretical conclusions drawn from data are often variable even when researchers have access to reliable data. <!-- repetition "data" -->
...
Furthermore, correlates of the contrast can even be found away from the consonant, in temporally distant speech units. <!-- Still not sure why in this example we refer to a lexical contrast when it is clear we are focusing on the segment. Does it make sense to say the acoustic features can differ depending on the position of the consonant in the word? It wouldn't be the same lexical contrast if the segment was in another position... they would be different words and maybe no longer a minimal pair. -->
...
Differences in position, shape, and alignment of pitch modulations over multiple locations within a sentence are correlated with differences in discourse functions [e.g., @niebuhr2011there].
...
If the competitor and target objects differ but their color did not (e.g. *yellow banana* vs *yellow tomato*), the noun is in focus (the Noun Focus condition, NF).
...
For the present study, N analysis teams will receive access to the entire data set generated by the production study and will be instructed to answer the following research question: *Do speakers acoustically modify utterances to signal atypical word combinations in referring expressions?*
... <!-- what's with the 3 periods? --> 
...
In this study, we will follow the data collection procedure proposed by @Parker2020.
The project involves the following five phases:

1. \textsc{Recruitment}: We will recruit independent groups of researchers to analyze the data and review data analyses.
...
An online landing page will provide a general description of the project, including a short pre-recorded slide-show that summarizes the data set and research question (<https://many-speech-analyses.github.io>).
The project will be advertised via social media, using mailing lists for linguistic and psychological societies, and via word of mouth.^[Terms in small caps in this and later sections are included with their definition in the glossary at the end of the paper for the reader's convenience.]
...
As outlined above, our aim is to assess the variability of the reported effects and how these are affected by a set of predictors, rather than the meta-analytic estimate of the investigated effect *per se*.
...
For those reported models that were originally fitted within a frequentist approach, uniform distributions will be used as the priors of all parameters (with the restriction that only positive numbers will be included for scale parameters), making the standardized models in fact equivalent to the reported frequentist models. <!-- It's a technicality but given that bayesian approximation of the posterior is based on sampling, this statement can't be true. I understand you are referring specifically to the MAP and that it would be the same as the MLE point estimate, but this reads as if youre saying the models are equivalent. --> 
...
All relevant data, code, and materials will be publicly archived on the Open Science Framework (<https://osf.io/3bmcp/>).^[Link for peer-review: <https://osf.io/3bmcp/?view_only=4f00dff704d742d493870f9f3d68e432>.]
