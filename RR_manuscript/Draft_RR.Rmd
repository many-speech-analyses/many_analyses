...
# number of analyses
# JVC: Why is this the number of total analyses and not nrow(msa_models)?
msa_models_unique <- msa_models %>%
  select(animal, model_n, outcome, typicality_operationalization, framework, model, temporal_window, random_effects, n_random, phon_rating, stat_rating, n_predictors, pop_pred, operationalisation, operationalisation_notes) %>%
  distinct()
...
unique_acou_stat <- msa_models %>%
  select(model_n, outcome, typicality_operationalization, framework, model, temporal_window, random_effects, n_random, phon_rating, stat_rating, n_predictors, pop_pred, operationalisation, operationalisation_notes) %>%
  unnest(pop_pred) %>%
  distinct()
...
tbl_team_desc <- msa_models %>% 
  ...
  mutate(across(-c("metric", "min", "max"), specify_decimal, k = 2)) %>% 
  mutate(desc = "team_desc", 
    metric = fct_relevel(metric, "years_from_phd"), 
  ...
  add_row(label = " ", desc = "team_desc", Metric = " ", 
          Min. = NA, Max = NA, `Mean (SD)` = NA, avg = NA, sd = NA, Range = NA) %>% 
  mutate(across(-c("label", "desc", "Metric"), ~replace(., is.na(.), " ")))
...
# Combine in single tibble (for printing in prose and table)
tbl_team_model_desc <- bind_rows(tbl_team_desc, tbl_model_desc) %>% 
  mutate(desc = case_when(
    Metric != "Overall peer review rating" & desc == "team_desc" ~ " ", 
    Metric != "n. coefficients" & desc == "model_desc" ~ " ", 
    TRUE ~ desc), 
    desc = case_when(
      desc == "team_desc" ~ "Teams", 
      desc == "model_desc" ~ "Models", 
      TRUE ~ " ")
    ) 

tbl_team_model_desc %>% 
  select(Descriptives = desc, Metric, Range, `Mean (SD)`) %>% 
  knitr::kable(format = "latex", booktabs = T, escape = F, 
    linesep = "", align = c("l", "l", "c", "r"), 
    caption = "Descriptive statistics for teams and models included in the 
    meta-analysis (n = 30).",
    label = "team-descriptives-table"
    ) %>% 
...
tbl_categorical_desc <- msa_models %>% 
  ...
  mutate(across(c("prop", "perc"), specify_decimal, k = 2)) %>% 
...
tbl_categorical_desc %>% 
  ...
  mutate(Metric = case_when(
    Type != "Bayesian" & Metric == "Framework" ~ " ", 
    Type != "GAM" & Metric == "Model" ~ " ", 
    Type != "Duration" & Metric == "Outcome" ~ " ", 
    Type != "Other" & Metric == "Temporal window" ~ " ", 
...
# pull_from_tib(tbl_categorical_desc, label, "framework_bayesian", n)
