...
In a seminal collaborative effort, @silberzahn2018many let twenty-nine independent analysis teams address the same research hypothesis: whether soccer referees are more likely to give red cards to dark-skin-toned players than to light-skin-toned players.
...
Building on the many-analysts approach, @landy2020crowdsourcing asked 15 research teams to independently design studies to answer the same research questions.
...
Once we have decided to compare fundamental frequency (f0) of the word *another* across the two utterances, there are still many choices to be made, all of which need to be justified.
...
Each analysis was evaluated by four different teams who functioned as peer-reviewers.
...
The major measurement dimension of each analysis was categorized according to the following possible groups: *duration*, *intensity*, *fundamental frequency* (f0), *other spectral properties* (e.g. frequency, center of gravity, harmonics difference, etc.), and *other measures* (e.g. derived measures such principal components, vowel dispersion, etc.).
...
The research compendium of the study, containing all the code and data presented here, can be found at <https://osf.io/3bmcp/>.
...
Analysts had on average `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", avg)` years (SD = `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", sd)`) of experience after completing their PhD, ranging from `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", Min.)` years, i.e. PhD students (or less experienced) to `r pull_from_tib(tbl_team_desc, label, "team_desc_years_from_phd", Max)` years, suggesting that, on average, analysts were experienced researchers.
...
At the end of Phase 2 (primary data analysis), the teams had submitted a grand total of `r no_analyses` individual models to answer the research question, with a mean of `r pull_from_tib(tbl_model_desc, label, "model_desc_models", avg)` (SD: `r pull_from_tib(tbl_model_desc, label, "model_desc_models", sd)`) models per team. 
...
`r (round(outcome_n[[2,1]],2) * 100) %>% xfun::numbers_to_words() %>% str_to_sentence()` percent of the models used fundamental frequency (f0) as the outcome variable, `r (round(outcome_n[[1,1]],2) * 100)`\% used a measure of duration, `r round(outcome_n[[3,1]],2) * 100`\% used vowel formants, `r round(outcome_n[[4,1]],2) * 100`\% intensity, and `r round(outcome_n[[5,1]],2) * 100`\% other measures.
...
This means that, in addition to the critical predictor (typicality of the adjective noun combinations), models had on average one additional predictor (range = `round(predictor_n$min_predictor,1)`-`round(predictor_n$max_predictor,1)`).
...
Teams differed with regard to how the acoustic signal was segmented ranging from fully automated forced-alignment with minimal manual correction to complete manual alignment performed by the analysts; teams differed in whether the statistical analysis was based on a subset of the data or the whole dataset; and they differed whether and if so how measurements were excluded both qualitatively (i.e. whether specific speech production instances were excluded or not) and quantitatively (i.e. whether data were trimmed or not).

The question arises whether teams arrived at different conclusions. 
Yes, they did.
`r n_atleast_1 %>% xfun::numbers_to_words() %>% str_to_sentence()` teams out of the `r length(unique(msa_models$animal)) %>% xfun::numbers_to_words()` (`r perc_atleast_1`%) reported to have found at least one statistically reliable effect (based on the inferential criteria they themselves specified).
...
<!-- TODO: repeat exclusion criteria -->
...
In other words, the estimated range of variation across teams lies somewhere between `r pm(round((meta_bm_prereg_tidy$conf.low[['sd_animal__Intercept']] * 1.96), 2))` (0.03 * 1.96) and `r pm(round((meta_bm_prereg_tidy$conf.low[['sd_animal__Intercept']] * 1.96), 2))` (0.07 * 1.96) with 95% credibility.”
...
This analysis was not preregistered and should thus be interpreted with caution.^[Note that before fitting this model, we fit a separate one in which model ID was the only (non-nested) group-level effect. The estimated group-level effect of model ID is identical to that of the nested model, so we will not discuss it further.]
...
<!-- TR: The meta analytical estimate is basically meaningless, given that the scale is not interpretable and artificial negative and positive estimates cancel each other out. I wonder whether we should take this out --> 
...
We will interpret these standard units relative to phonetic measures in the discussion section, but to provide an intuition about the absolute scale of variation, we can express them on a standardized scale that does not require domain expertise: The IQ scale [@kamphaus2019clinical]. 
The observed variability between teams corresponds to a difference in IQ up to 14.9 IQ points.
...
```{r plot-meta1, echo = FALSE, fig.cap="Standardized effect sizes across all critical coefficients provided by the teams. Raw estimates are displayed in grey. Estimates after shrinkage as provided by the meta-analytical model are displayed in black/orange.", out.width="100%"}
...
Since there are combinations of analytical choices that appear to systematically result in lower or higher estimates and the fact that predictors are not fully crossed (i.e. we do not have the same amount of data for all combinations of e.g. outcome and temporal window), the estimates for certain predictors might be biased if predictors are collinear. 
...
```{r plot-meta2, echo = FALSE, fig.cap="The effects of analytic and researcher-related predictors on the reported standardized effect sizes. (A) Posterior samples for the four most frequent outcome variables; (B) Posterior samples for the four most frequent temporal windows: Black point indicates median, and shaded shape represents 50/80/95% highest density interval. (C) Mean posterior samples and 80/95% credible intervals for all predictors", out.width="100%"}
...
operationalizing parameters of a multidimensional signal, operationalizing and choosing appropriate model predictors, and constructing appropriate statistical models.
...
Note that this is a conservative estimate of the number of possible analytical choices for our research question, ignoring many other degrees of freedom like e.g. acoustic parameter extraction, outlier treatment, and transformations, all of which might have an impact on the final results [@breznau2021observing].

To gain a better understanding of whether the observed variability can result in theoretically different claims, we will contextualize them in actual acoustic measures.
...
We can now use these values to interpret the meta-analytic estimates, which are in standardized units, by transforming the standardized units to measures of duration, fundamental frequency and intensity.^[Note that these categories necessarily refer to variegated sets of measures. E.g. the domain word includs words that differed along several dimensions, including their length and their metrical structure.]
...
Vowel duration differences within this range have also been reported across phenomena associated with segmental contrasts [@coretta2019], reduction [@nowak2006], and biomechanical reflexes of prominence [@mucke2014effect]. 
Thus, variation between different analysts of `r round(3.8*1.96)` to `r round(7.7*1.96)` ms in one or the other direction can be theoretically relevant and might lead to opposing theoretical conclusions.
...
Furthermore, the observed variation does not seem to be systematic.
...
It is important to note that we do not know whether there is an acoustic manifestation of typicality in the first place. 
Looking at our data, one might argue (and this observation has been made by several teams during the collaborative write-up) that our sample of speech scientists converged on a qualitative conclusion, i.e.  there is no relationship. 
However, if there truly is no underlying relationship, our results would suggest a concerning false positive rate with `r perc_atleast_1`% of teams reported to have found at least one statistically reliable effect. This rate is substantially higher than the conventionally accepted 5% false positive rate in for example null hypothesis significance testing frameworks.
...
If the latter was true, the fact that the majority of teams converged on a null result might also simply be a consequence of the sample size in the data set being too small to reliably detect an effect. 
Thus, we do not think that our study provides convincing evidence that speech researchers converged on the same qualitative answer to the research question. 
...
For example, measurements related to fundamental frequency tended to result in lower estimates while measurements related to duration tended to yield higher estimates.
...
While this is a valid way to explore data and generate hypotheses, it is not suitable to test hyopotheses.
...
However, less clear-cut degrees of freedom such as observed in the present study can not be corrected for in a straightforward way. They can nevertheless drastically inflate the false positive rate  [@roettger2019researcher]. 
...
Some of the analysis teams explicitly recognized and acknowledged the need to formulate a more precise research question by preregistering their planned data analysis pipeline.  
...
Such an analysis can provide insight into how much the conclusions change due analytical choices as well as which choices have the largest impact on the result. This approach is called a "multiverse analysis" [e.g, @steegen2016increasing; @harder2020multiverse] and has recently gained popularity across disciplines.
...
Possible tool to strengthen quantitative theoretical development are relate to mathematically formalizing verbal expectations or using computational models [e.g., @van2020formalizing; @guest2021computational; @scheel2021hypothesis; @devezer2021case]. 
...
Beyond possible variation in how the research question is statistically interpreted, the analysts might have tried to answer different specific manifestations of the research question that was given to them (e.g. Do speakers modify f0 in atypical adjectives?). 
...
Third, our design has artificially inflated the observed variability by encouraging  anti-coordination strategies. 
...
The sampling strategy might also have given access to researchers who where not experienced enough in particular aspects of the acoustic analysis, possibly introducing uncommon analytical choices or poor quality analyses.
However, to our knowledge, neither the peer review among teams nor the information gathered through our questionnaires indicated any obvious cases of quality outliers. 
...
They have in the past used the lack of standardized analytical approaches as an argument for proposing behavior and policy changes in the field. 
This might have biased their own judgement during the analysis which itself came with many researcher degrees of freedom.
We hope we were able to make these degrees of freedom as well as the timing and reasoning of these analytical choices at least detectable and we invite other researchers to re-analyze our data and try to replicate our results using a different research question.
...
This is an important point to make because cognitive sciences in general, and the language sciences in particular, have many research areas that are based on qualitative methods.
...
When many different analysts have to analyze the same data set to answer the same research question, analysts differ drastically in how they approach this task, leading to both different qualitative answers (i.e. is there evidence for a relationship or not) and vastly different effect magnitudes. 
...
Given the observed variability, conservative estimates of the sheer number of possible analytical paths for this research question lies in the thousands.
