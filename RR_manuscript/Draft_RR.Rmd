...
#tbl_model_desc <- msa_models_unique %>% 
#  select(animal, starts_with("n_")) %>% 
#  group_by(animal) %>% 
...
#    label = glue::glue("{desc}_{metric}")) %>% 
#  select(label, desc, Metric, Min. = min, Max = max, `Mean (SD)`, avg, 
#    sd, Range) %>% 
#  mutate(across(-c("desc", "Metric"), as.character))
...
`r (pull_from_tib(team_desc_df, stage, "Initial", n_teams) - pull_from_tib(team_desc_df, stage, "Final", n_teams)) %>% xfun::numbers_to_words() %>% stringr::str_to_sentence()` of the signed-up teams dropped out during the analysis phase.

`r pull_from_tib(team_desc_df, stage, "Final", n_teams) %>% xfun::numbers_to_words() %>% stringr::str_to_sentence()` teams submitted their analysis by the established deadline.
These are the teams that are included in this study.
The submitting teams comprised `r pull_from_tib(team_desc_df, stage, "Final", n_analysts)` analysts, with an average of `r pull_from_tib(tbl_team_desc, label, "team_desc_size", avg)` (SD = `r pull_from_tib(tbl_team_desc, label, "team_desc_size", sd)`) individuals per team.
