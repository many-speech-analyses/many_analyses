...
In conjunction with an academic incentive system that rewards certain discovery processes more than others [@sterling1959publication; @koole2012rewarding], we often find ourselves exploring many possible analytical pipelines, but only reporting a select few. <!-- in connection with = referring to; in conjunction with = together, coupled with. I think we want the later, but change if I'm mistaken (JVC) --> 
...
Recent collaborative attempts have started to shed light on how different analysts tackle the same data set and have revealed a large amount of variability. <!-- I think this should be part of the previous paragraph. --> 
...
This lexical contrast is manifested by many acoustic features which can differ depending on several factors, such as position of the consonant in the word and surrounding sounds [@lisker1977rapid]. <!-- why not "phonological contrast"? --> 
...
When looking at phrase-level temporal domains, the number of possible analytical pipelines increases exponentially. <!-- this is hyperbole, but seems (possibly) more appropriate than "explodes" -->
This plurality of analytical paths is illustrated in Figure \@ref(fig:forkingPaths).
<!-- When comparing two utterances such as (2a) and (2b), there are many things to consider.  (I think we can do without this sentence.) -->
...
Figure \@ref(fig:forkingPaths) \highlight[id=SC, comment = {The figure text is quite small. Probably better to change to a longer rather then wider layout.}]\highlight[id=TR, comment = {Better now?}], illustrates a specific option to automatically calculate f0\highlight[id=TR, comment = {add examples of pitch tracking options}.
Knowing that the signals and the measures thereof can be noisy, another option would be to smooth them using one algorithm and degree over others, or remove values that substantially deviate from surrounding values or expectations, either manually or automatically, and so on. <!-- Im having a bit of trouble with this sentence. I tried to fix it and now I am realizing I don't quite follow (JVC) --> 
...
Moreover, the data is not available, and the statistical analysis used anti-conservative co-variance structures, leaving reason to doubt the original conclusions. <!-- "lending doubt on" sounded a bit weird to me (JVC) -->
...
The initiating authors (SC, JC, TR) created an online landing page providing a general description of the project [https://many-speech-analyses.github.io](https://many-speech-analyses.github.io) and a short pre-recorded slide show that summarizes the study and research question (LINK). <!-- same as above, or OSF -->
...
We will summarize the variability in reported effect sizes using Bayesian meta-analytic \highlight[id=SC, comment={It would be good to decide on a clear terminology to refer to the individual original models, the individual models that we run (to refit the original models), and the meta-analytical model}]{techniques}.
...
\highlight[id=SC, comment = {Is this sentence good?}]{Factor-level ordering} for each categorical variable will mirror that of the original analyses.
<!-- TR: don't know what you mean tbh; JVC: I get what you're saying, essentially we will keep the levels of their categorical variables the same when we refit the models, right? I changed it so that it says that. Change if wrong. -->
...
<!-- I understand "posterior probability" to refer to a point estimate, i.e., the posterior probability that B > 0 = some value between 0 and 1. I may be mistaken, but I think you are referring to the posterior distribution in the above paragraph -->
