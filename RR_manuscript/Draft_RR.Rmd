...
These studies, however, might still underestimate the extent to which analysts vary because data analysis is not merely restricted to how already obtained numbers are statistically analyzed.
Those numbers are usually the result of a complex measurement process that translates human behavior into numbers.
...
Our goals are twofold: First, our study conceptually replicates previous many-analyses projects, probing previously observed flexibility in statistical analyses in order to independently confirm previously published findings and probe its generalizability to other disciplines (here speech sciences). 
Second, our study extends previous findings and estimates the amount of observed flexibility in quantifying complex human behavior (here human speech). 
This is important because previous studies might have investigated only the lower bound of variability across analysts because analysts did not have to quantify the variables under investigation but were instead already provided with numerical data.
