...
In this article, we investigate the impact of analytic choices on research results when many analyst teams examine the same speech production data set, a process that involves both decisions regarding the \emph{operationalization} of a complex signal and decisions regarding \emph{statistical analysis}.
...
Consequently, a good part of published papers likely present overconfident interpretations of data and statistical results based on idiosyncratic analytic strategies \citep[e.g.,][]{simmons2011false, gelman2014statistical}.
...
The research compendium of the study containing all the code and data presented here and can be found at \url{https://osf.io/3bmcp/}.
...
Of the submitted analyses, the initiating authors identified 170 from 30 teams that were deemed eligible to be included in the meta-analytic synthesis.
...
At the end of Phase 2 (primary data analysis), the teams had submitted a grand total 104 individual models to answer the research question, with a mean of 3.6 (SD: 3.4) models per team.
...
Thirty-five percent of the models used a measure of duration as the outcome variable, 36\% used fundamental frequency (f0), 12\% used vowel formants, 14\% intensity, and 3\% other measures.
...
Eight percent of the linear models specified no random effects at all (without pooling their data), so effectively ignoring these non-independences \citep{hurlbert1984pseudoreplication}.
...
This means that the mean deviations of any individual team from the meta-analytic effect estimate range between \(\pm 0.06\) to \(\pm 0.14\) (0.3 * 1.96, 0.07 * 1.96) in standard units at 95\% probability.
...
The nested model yields a posterior 95\% CrI for between-team variability of 0 to 0.05 standard units (\(\beta\) = 0.02, SD = 0.01), corresponding to a mean deviation range of about \(\pm 0.1\) standard units and 95\% probability.
...
While the majority of model estimates and their uncertainty after shrinkage (black, orange, and blue points and lines) yields inconclusive results, there are 27 model estimates for which the 95\% credible interval does not contain zero (16\%).
...
Being aware of this idiosyncratic variation between analysts should lead to more nuanced claims and what \citet{breznau2021observing} refer to as ``epistemic humility'' (p.~10).
...
This uncertainty is possibly further inflated by the fact that the research question presented to the teams was vague, despite being of a kind normally found in the speech science literature: \emph{Do speakers acoustically modify utterances to signal atypical word combinations?}
...
However, we consider this very underspecification of research hypotheses in the field of speech science \citep[and beyond, see][]{scheel2022most} a common phenomenon.
