...
At the end of Phase 2 (primary data analysis), the teams have submitted a grand total 181 individual models to answer the research question, with a mean of 3.6 models per team.
...
34\% of models used a measure of duration as the outcome variable, 39\% used fundamental frequency (f0), 13\% used vowel formants, 13\% intensity, and 2\% other measures.

41\% of models obtained acoustic measures at the level of the segment (e.g.~comparing the acoustic profile of a vowel), 49\% from the word level (e.g.~comparing the acoustic profile of \emph{Banane} `banana'), 3\% at the level of the phrase (e.g.~the noun phrase including determiner and adjective, e.g.~``the green banana''), 2\% from the whole sentence, and 3\% used a different time window.
Based on a coarse coding of how acoustic measures were operationalized, we find a total of 54 different measurement specifications.
...
Among the possible operationalizations, 80\% of models contained typicality as a categorical variable (e.g.~atypical vs.~typical), 17\% used a continuous typicality scale from 0-100 by calculating the mean typicality for each word combination as obtained from the norming study, while 2\% of the models used the median typicality rating.

The majority of models were run within a frequentist framework (81\%).
19\% were run within a Bayesian framework.
...
6\% of linear models specified no random effects at all (without pooling their data), so effectively ignoring these non-independences \citep{hurlbert1984pseudoreplication}.
63\% specified random intercepts only, and 31\% specified random intercepts and random slopes to account for the non-independence.
On average, teams that specified random effects, included 2.4 random terms in their models.
Based on statistical framework, type of model, distribution family, fixed terms, and discarding random effects, there were a total of 44 different model specifications.
...
The mean rating of the acoustic analyses, on a scale from 0 to 100, is 74.9 (SD = 11.3).
The mean rating of the statistical analysis is 74 (SD = 13.8).
...
In the first model, the meta-analytic intercept estimate is between -0.01 and 0.025 standard units (95\% CrI, mean = 0.008).
...
Notably, while the majority of models yields inconclusive results, there are 21 model estimates for which the 95\% credible interval does not contain zero (12\%).
