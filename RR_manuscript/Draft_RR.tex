...
All individuals interested in participating were asked to complete a questionnaire detailing their familiarity with numerous analytical approaches common in the speech sciences (\texttt{analytic\_approach\_quest.pdf}).
...
We then requested that they answer the question: \emph{Do speakers acoustically modify utterances to signal atypical word combinations?}
...
This same sequence was followed until all analyses allocated to a given reviewer were provided and reviewed.\footnote{We initially planned to simultaneously present all four (or more) methods sections to each reviewer after the fourth round, with the option to revise their original ratings and provide an explanation. We have decided to skip this last step due to time constrains.}
...
We fitted this model with 4 chains of Hamiltonian Monte-Carlo sampling for the estimation of the joint posterior distribution, using the No U-Turn Sampler (NUTS) as implemented in Stan \citep{stan2021}, and 4000 iterations (2000 for warm-up) per chain, distributed across 8 processing cores and 2 threads in within-chain parallelization.
...
The code used to run the model can be found at \textless\textgreater.
...
The research compendium of study containing all the code and data presented here and can be found at \url{https://osf.io/3bmcp/}.
...
Of the submitted analyses, the initiating authors identified 170 from 30 teams that were deemed eligible to be included in this study.
...
The included analyses were provided by 30 teams, comprising 109 analysts, with an average of 3.6 (SD = 2.8) individuals per team.
...
Analysts had on average 4.8 years (SD = 4.2) of experience after completing their PhD, ranging from -3.8 years, i.e.~PhD students (or younger) to 12.4 years, suggesting that, on average, analyst were experienced researchers.
...
At the end of Phase 2 (primary data analysis), the teams have submitted a grand total 104 individual models to answer the research question, with a mean of 3.6 (SD: 3.4) models per team.
...
When considering both acoustic and statistical analysis, we have found a grand total of 104 different analytic pipelines.
...
\toprule
\bf{Team characteristics} &    & \bf{Range} & \bf{Mean (SD)}\\
\midrule
 & Team size & 1.0 -- 12.0 & 3.6 (2.8)\\
 & Years after phd & -3.8 -- 12.4 & 4.8 (4.2)\\
 & Prior belief & 48.5 -- 92.0 & 69.2 (11.3)\\
 & Acoustic analysis peer rating & 41.2 -- 88.3 & 71.5 (13.9)\\
 & Statistical analysis peer rating & 33.0 -- 93.3 & 69.5 (16.5)\\
 & Overall peer rating & 39.0 -- 88.7 & 70.0 (14.3)\\
 &  &  \vphantom{5} & \\
\bf{Acoustic analyses} &  & \bf{n} & \bf{\%}\\
\midrule
\hspace{1em}Outcome & F0 & 37 & 35.6\\
 & Duration & 36 & 34.6\\
 & Intensity & 15 & 14.4\\
 & Formants & 13 & 12.5\\
 & Other & 3 & 2.9\\
 &  &  \vphantom{4} & \\
\hspace{1em}Temporal window & Segment & 47 & 45.6\\
 & Word & 45 & 43.7\\
 & Sentence & 4 & 3.9\\
 & Phrase & 3 & 2.9\\
 & Other & 4 & 3.9\\
...
\hspace{1em}Model & Linear model & 102 & 98.1\\
 ...
 &  &  & \\
 &  & \bf{Range} & \bf{Mean (SD)}\\
\hspace{1em}N & Models & 1 -- 16 & 3.6 (3.4)\\
 & Predictors & 1 -- 5 & 2.0 (1.2)\\
 & Random terms & 1 -- 10 & 2.5 (1.3)\\
\hspace{1em} & \hspace{1em}Intercept & \hspace{1em}1 -- 10 & \hspace{1em}1.9 (1.0)\\
\hspace{1em} & \hspace{1em}Slope & \hspace{1em}0 -- 4 & \hspace{1em}0.5 (0.9)\\
\bottomrule
\end{tabular}}
\end{table}
...
The mean rating of the acoustic analyses, on a scale from 0 to 100, is 71.5 (SD = 13.9, see Table \ref{tab:descriptives-table}).
...
\hypertarget{meta-est}{%
\subsection{Meta-analytic estimation}\label{meta-est}}
...
Estimates ranged between -0.65 to 0.99 standard units.
...
Neither analysts' prior beliefs in the phenomenon (\(\beta\) = 0.00, 95\% CrI = {[}-0.03, 0.03{]}), nor their seniority in terms of years after completing their PhD (\(\beta\) = 0.01, 95\% CrI = {[}-0.03, 0.04{]}) seem to compellingly affect model estimates.
...
Interestingly, the model uniqueness, i.e.~how unique the choice and combination of predictors is, affects the analysts estimate, with more unique models producing higher positive estimates (\(\beta\) = 0.05, 95\% CrI = {[}0.02, 0.07{]}).
...
The reported effect estimates from teams varied between -0.65 and 0.99 standard units which corresponds to duration values ranging from -21.86 to 50.81.
...
For example, the phenomenon of incomplete neutralization, a widely studied phenomenon, involves duration effects ranging from 7 to 15 ms \citep{nicenboim2018using}. This particular phenomenon has sparked long-lasting methodological and theoretical debates about the very nature of linguistic representations \citep{port2005against}.
Vowel duration differences within this range have been reported across phenomena, associated with segmental contrasts \citep{coretta2019}, reduction \citep{nowak2006}, and biomechanical reflexes of prominence \citep{mucke2014effect}.
Expecting random variation between different analysts of 7 to 15 in one or the other direction can be theoretically relevant and might lead to opposing theoretical conclusions.
...
Many analysts targeted measures related to fundamental frequency (f0), likely because similar functional relationships like information structure and predictability can be expressed by f0 \citep[e.g.][]{grice2017integrating, turnbull2017role}.
...
The complexity of the speech signal lends itself to multiple approaches, but this plurality of hypothesis tests invites bias and can dramatically increase the rate of falsely claiming the presence of an effect \citep[\citet{simmons2011false}]{roettger2019researcher}.
One team coordinator gave the following reason for dropping out of the project (and allowed us to share this statement): ``{[}T{]}he project's approach is too much top-down instead of bottom-up. {[}\ldots{]} I would rather prefer to explore the collected utterances and run different types of acoustic analyses, which would then be followed by statistical evaluation of the data.''
...
Another possible outcome of analytic flexibility as seen in this study is selective reporting of those tests that yield a desirable outcome \citep[\citet{john2012measuring}, \citet{simmons2011false}]{kerr1998harking}, while null results remain unreported \citep[\citet{rosenthal1979file}]{sterling1959publication}.
...
The replication crisis has brought attention to scientific practices that lead to unreliable and biased claims in the literature \citep[\citet{fidler2018reproducibility}]{vazire2017quality}.
One of the suggested paths forward is for researchers to directly replicate previous studies more often \citep[\citet{camerer2018evaluating}]{open2015estimating}.
...
Beyond a replication crisis, quantitative disciplines are going through what has been called an ``inference crisis'' \citep[\citet{starns2019assessing}]{rotello2015more}.
...
Within fields that deal with speech data, open source software that allows to extract acoustic parameters via reproducible scripts can help other researchers to trace back seemingly inconsequential choices during the measurement process \citetext{\citealp[e.g., Praat:][]{boersma2021praat}; \citealp[EMU:][]{winkelmann2017emu}; \citealp[the Montreal Forced Aligner:][]{mcauliffe2017}}.
...
In light of these possible sources of bias, a field can benefit from explicit positionality statements \citep[e.g.,][]{jafar2018, darwin2020, fox2021open}.
