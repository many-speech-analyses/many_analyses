...
Of the analysis submitted by the 46 teams, the initiating authors identified analyses from 32 teams to be eligible to be included in the meta-analytic model.
...
The included analyses were provided by 32 teams, comprising 117 analysts, with a median of 3.0 individuals per team.
...
At the end of Phase 2 (primary data analysis), the teams had submitted a grand total of 119 individual models (including 192 critical model coefficients, given that some models returned more than one critical coefficient) to answer the research question, with a median of 3 models per team.
...
Thirteen teams out of the thirty-three (39.3939394\%) reported to have found at least one statistically reliable effect (based on the inferential criteria they specified).
Of the 170 critical model coefficients, 45 were claimed to show a statistically reliable effect (23.4\%).

\begin{table}

\caption{\label{tab:descriptives-table}Descriptive statistics of teams, acoustic analyses, and statistical analyses included in the meta-analysis. The data set included analyses from 32 teams and  117 analysts.}
\centering
\resizebox{\linewidth}{!}{
...
In other words, the estimated range of variation across teams lies somewhere between \(\pm 0.06\) (0.03 * 1.96) and \(\pm 0.13\) (0.07 * 1.96) with 95\% credibility.''
...
While the majority of model estimates and their uncertainty after shrinkage (in black) yields inconclusive results (i.e.~are compatible with a point null hypothesis), there are 27 model estimates for which the 95\% credible interval does not contain zero (in blue, 14\%).
...
Different analysis paths led to different categorical conclusions with 39.3939394\% of teams reported to have found at least one statistically reliable effect.
...
The reported effect estimates from teams varied between -0.7 and 1.01 standard units, which corresponds to duration values ranging from -23.34 to 52.07 ms.
...
However, if there truly was no underlying relationship, our results would suggest a concerning false positive rate with 39.3939394\% of teams reported to have found at least one statistically reliable effect.
...
If, on the other hand, there actually was an underlying relationship, our results would suggest a concerning false negative rate of -38.3939394\%, with the majority of teams not detecting the effect.
