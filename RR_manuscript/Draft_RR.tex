...
Extending on the many-analysts approach, \citet{landy2018crowdsourcing} asked 15 research teams to independently design studies to answer the same research questions.
...
This is in line with a recent re-analysis of the study by \citet{silberzahn2018many} by \citet{auspurg2021has}.
The authors argue that the observed heterogeneity across analysts in \citet{silberzahn2018many} might be driven by flexibility in interpreting the research question.
...
This account, however, has mainly focused on content selection \citep{gatt2013we}.
...
The two sentence prompts were used to create a focus contrast between the competitor and the target object. Focused units denote the set of all (contextually relevant) alternatives \citep[e.g.][]{rooth1992theory}.Concretely, a focus contrast encodes information that is assumed to be contrary to the presuppositions of the interlocutor.
For instance, if the competitor and target objects differ but their color did not (e.g.~\emph{yellow banana} vs \emph{yellow tomato}), the noun is in focus (the Noun Focus condition, NF).
