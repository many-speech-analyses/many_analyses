...
- num: 20
  org: University of Pennsylvania, United States
- num: 21
...
Further documents concerning the collaborative editing of the Registered Report can be found at <https://drive.google.com/drive/folders/1-DOcj1qtEkvWfzu_FrsxkIGfPS0DyLXB?usp=sharing>.
...
team_desc_df <- bind_rows(
  ...
  team_leaders %>% 
    left_join(select(teams, animal, leader = coordinator), by = "leader") %>% 
    filter(leader %in% teams_used_in_analysis) %>% 
    group_by(animal) %>% 
    mutate(member_num = seq_along(ResponseId)) %>% 
    group_by(animal) %>% 
    mutate(n_members = max(member_num)) %>% 
    distinct(animal, n_members) %>% 
    ungroup() %>% 
    summarize(n_teams = nrow(.), 
              n_analysts = sum(n_members), 
              median_size = median(n_members), 
...
# model
# multivariate are counted as individual models
...
An interactive web application that allows the interested reader to explore the data set is available at <https://many-speech-analyses.github.io/shiny>. 
...
Of the analyses submitted by the `r pull_from_tib(team_desc_df, stage, "Submitted", n_teams)` teams, the initiating authors identified `r pull_from_tib(team_desc_df, stage, "Final", n_teams)` teams with submissions meeting the criteria for inclusion in the meta-analytic model.
...
A sankey diagram illustrating the relationship between choices related to outcome, temporal window and operationalization can be found in the supplementary materials at <https://many-speech-analyses.github.io/many_analyses/RR_manuscript/supplementary_materials.pdf>.
...
`r n_atleast_1 %>% xfun::numbers_to_words() %>% str_to_sentence()` teams out of the `r length(unique(msa_models$animal)) %>% xfun::numbers_to_words()` (`r round( perc_atleast_1, 1)`%) reported to have found at least one statistically reliable effect (based on the inferential criteria they specified).
Of the `r nrow(msa_models)` critical model coefficients, `r n_effects` were claimed to show a statistically reliable effect (`r round(perc_effects,1)`%). 
...
In other words, the estimated range of variation across teams lies somewhere between `r pm(round((meta_bm_prereg_tidy$conf.low[['sd_animal__Intercept']] * 1.96), 2))` (0.03 * 1.96) and `r pm(round((meta_bm_prereg_tidy$conf.high[['sd_animal__Intercept']] * 1.96), 2))` (0.07 * 1.96) standard units with 95% credibility.
...
This analysis was not preregistered and should thus be interpreted with caution.^[Note that before fitting this model, we fitted a separate one in which model ID was the only (non-nested) group-level effect. 
...
In other words, our best guess is that speakers might not encode typicality in the acoustic signal (e.g. by duration, f0, etc,) or, if they do, they do so by a maximum of `r pm(x = 0.03)` standard units.
...
This suggests that the acoustic measures of typical word combinations are `r -round(meta_intercept_nest[3],2)` standard units lower to `r round(meta_intercept_nest[4],2)` standard units higher than the measures of atypical word combinations, at 95% confidence.
...
While the majority of model estimates and their uncertainty after shrinkage yields inconclusive results (i.e. are compatible with a point null hypothesis), there are `r compelling_count_nest[[1]] + compelling_count_nest[[3]]` model estimates for which the 95\% credible interval does not contain zero (`r round((compelling_results_nest[[1,1]] + compelling_results_nest[[3,1]]), 2) * 100`\%).

<!-- FIGURE -->
(ref:plot-meta1) Standardized effect sizes across all critical coefficients provided by the teams. Raw estimates are displayed in grey. Estimates after shrinkage as provided by the meta-analytic model are displayed in black.
...
Different analysis paths led to different categorical conclusions with `r round(perc_atleast_1, 1)`% of teams reported to have found at least one statistically reliable effect.
...
```{r ms}
meta_low <- round(meta_bm_nest_tidy$conf.low[["sd_animal:model_id__Intercept"]], 2)
meta_high <- round(meta_bm_nest_tidy$conf.high[["sd_animal:model_id__Intercept"]], 2)

ms_low <- round(min(msa_models$estimate) * round(msa_sd$sd_min[1], 1), 2)
ms_high <- round(max(msa_models$estimate) * round(msa_sd$sd_min[1], 1), 2)

ms_sd_min <- round(msa_sd$sd_min[1], 1)
ms_sd_max <- round(msa_sd$sd_max[1], 1)

ms_meta_low <- round(ms_sd_min * meta_low * 1.96, 1)
ms_meta_high <- round(ms_sd_max * meta_high * 1.96, 1)
```


For example, for those analyses that investigated the duration of vowels (e.g. the duration of the stressed vowel in *Banáne*), the reported duration measures exhibit standard deviations that range from `r ms_sd_min` to `r ms_sd_max` ms.
...
The reported effect estimates from teams varied between `r round(min(msa_models$estimate),2)` and `r round(max(msa_models$estimate),2)` standard units, which corresponds to estimated segment duration differences (for atypical vs typical combinations) ranging from `r ms_low` to `r ms_high` ms.
A more conservative approach is to convert the meta-analytic estimates of between-model variation, thus obtaining an estimate of between-model variability in milliseconds that ranges between `r ms_meta_low` and `r ms_meta_high` ms at 95% credibility.
The calculation is thus: the minimum standard deviation of duration multiplied by the lower limit of the 95% CrI of the between-models variability estimate, times 1.96 to obtain a 95% CrI: `r ms_sd_min` * `r meta_low` * 1.96 = `r ms_meta_low` ms; the maximum standard deviation of duration multiplied by the upper limit of the 95% CrI of the between-models variability estimate, times 1.96: `r ms_sd_max` * `r meta_high` * 1.96 = `r ms_meta_high` ms.
...
Thus, variation between different analyst teams of `r ms_meta_low` to `r ms_meta_high` ms in one or the other direction can be theoretically relevant and might lead to opposing theoretical conclusions.
...
However, if there truly was no underlying relationship, our results would suggest a concerning false positive rate with `r round(perc_atleast_1, 1)`% of teams reported to have found at least one statistically reliable effect. 
...
If, on the other hand, there actually was an underlying relationship, our results would suggest a concerning false negative rate of `r round(100 - perc_atleast_1, 1)`%, with the majority of teams not detecting the effect. 
...
Thus beyond a mere replication crisis, quantitative disciplines are going through an "inference crisis" [@rotello2015more; @starns2019assessing].
...
Through sharing of materials, data and statistical protocols, we can make our idiosyncratic choices transparent to others [@munafo2017manifesto; @vazire2017quality].
...
Possible tools to strengthen theoretical development relate to mathematically formalizing verbal expectations or using computational models [e.g., @van2020formalizing; @guest2021computational; @scheel2021hypothesis; @devezer2021case].
...
See <https://github.com/many-speech-analyses/many_analyses/blob/main/figs/credit-taxonomy-all.png>.
...
Constantijn	Kaland, Martine Grice, Francesco Cangemi, Maria Lialiou, Malin Spaniol, Simon Wehrle: German Research Foundation (DFG) - Project-ID 281511265 - SFB 1252.
