...
%DIF DEL RR_stage_2.tex   Fri Jan 27 13:37:37 2023
%DIF ADD RR.tex           Mon Feb  6 13:27:23 2023
...
\DIFaddbegin \DIFadd{Relative to the registered protocol, we made minor changes to the refitting procedure, specifically file and variable naming conventions and the use of treatment contrasts instead of sum coding.
...
\DIFaddbegin \DIFadd{Further documents concerning the collaborative editing of the Registered Report can be found at }\url{https://drive.google.com/drive/folders/1-DOcj1qtEkvWfzu_FrsxkIGfPS0DyLXB?usp=sharing}\DIFadd{.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{We also did no include analyses for which we could not extract standardized effect sizes. }\DIFaddend For a list of \DIFdelbegin \DIFdel{exlcusion }\DIFdelend \DIFaddbegin \DIFadd{exclusion }\DIFaddend criteria, see Section Descriptive statistics below.
...
\DIFaddbegin \DIFadd{An interactive web application that allows the interested reader to explore the data set is available at }\url{https://many-speech-analyses.github.io/shiny}\DIFadd{.
}\DIFaddend 
...
Of the \DIFdelbegin \DIFdel{analysis }\DIFdelend \DIFaddbegin \DIFadd{analyses }\DIFaddend submitted by the 46 teams, the initiating authors identified \DIFdelbegin \DIFdel{analyses from 30 teams to be eligible to be included }\DIFdelend \DIFaddbegin \DIFadd{33 teams with submissions meeting the criteria for inclusion }\DIFaddend in the meta-analytic model.
...
\DIFaddbegin \DIFadd{Note that due to the unforeseen variability across teams, the latter exclusion criteria were not preregistered and were applied after having seen all analytic strategies.
}\DIFaddend 
...
The included analyses were provided by \DIFdelbegin \DIFdel{30 }\DIFdelend \DIFaddbegin \DIFadd{33 }\DIFaddend teams, comprising \DIFdelbegin \DIFdel{109 }\DIFdelend \DIFaddbegin \DIFadd{120 }\DIFaddend analysts, with a median of 3.0 individuals per team.
...
Analysts had a median of \DIFdelbegin \DIFdel{5.8 }\DIFdelend \DIFaddbegin \DIFadd{5.4 }\DIFaddend years of experience after completing their PhD, ranging from -3.8 years, i.e.~PhD students (or less experienced) to 12.4 years, suggesting that, on average, analysts were experienced researchers.
The analysts' prior belief in the effect under investigation, on a scale from 0 to 100, ranged from \DIFdelbegin \DIFdel{48.5 }\DIFdelend \DIFaddbegin \DIFadd{46.4 }\DIFaddend to 92.0 with a median of 70.0.
...
This means that, in addition to the critical predictor (typicality of the adjective noun combinations), models had on average one additional predictor (range = \DIFdelbegin \DIFdel{1-}\texttt{\DIFdel{r}}%DIFAUXCMD
...
\DIFdelbegin \DIFdel{Eight }\DIFdelend \DIFaddbegin \DIFadd{Nine }\DIFaddend percent of the linear models specified no random effects at all (without pooling their data), effectively ignoring these non-independences \citep{hurlbert1984pseudoreplication}.
\DIFdelbegin \DIFdel{Sixty-five }\DIFdelend \DIFaddbegin \DIFadd{Sixty-two }\DIFaddend percent specified random intercepts only, and \DIFdelbegin \DIFdel{27}\DIFdelend \DIFaddbegin \DIFadd{29}\DIFaddend \% specified both random intercepts and random slopes to account for the non-independence.
On average, teams that specified random effects included 2.5 random terms in their models.
Based on statistical framework, type of model, distribution family, fixed terms, and not including random effects, there were a total of \DIFdelbegin \DIFdel{47 }\DIFdelend \DIFaddbegin \DIFadd{52 }\DIFaddend different model specifications.

When considering both acoustic and statistical analyses, we have found a grand total of \DIFdelbegin \DIFdel{104 }\DIFdelend \DIFaddbegin \DIFadd{119 }\DIFaddend different analytic pipelines.
In other words, each individual analysis submitted was unique.
\DIFaddbegin \DIFadd{A sankey diagram illustrating the relationship between choices related to outcome, temporal window and operationalization can be found in the supplementary materials at }\url{https://many-speech-analyses.github.io/many_analyses/RR_manuscript/supplementary_materials.pdf}\DIFadd{.
}\DIFaddend 

Our quantitative assessment did not include other degrees of freedom, all of which are additional sources of variation:
Teams differed with regard to how the acoustic signal was segmented ranging from fully automated forced-alignment with minimal manual correction to complete manual alignment performed by the analysts; teams differed in whether the statistical analysis was based on a subset of the data or the whole \DIFdelbegin \DIFdel{dataset}\DIFdelend \DIFaddbegin \DIFadd{data set}\DIFaddend ; and they differed whether and if so how measurements were excluded based on both qualitative (i.e.~whether specific speech production instances were excluded or not) and quantitative grounds (i.e.~whether data were trimmed or not).

The question arises whether these unique analysis pipelines led to different conclusions.
\DIFdelbegin \DIFdel{Nine }\DIFdelend \DIFaddbegin \DIFadd{Thirteen }\DIFaddend teams out of the \DIFdelbegin \DIFdel{thirty (30}\DIFdelend \DIFaddbegin \DIFadd{thirty-three (39.4}\DIFaddend \%) reported to have found at least one statistically reliable effect (based on the inferential criteria they specified).
Of the \DIFdelbegin \DIFdel{104 submitted models, 37 were reported }\DIFdelend \DIFaddbegin \DIFadd{192 critical model coefficients, 45 were claimed }\DIFaddend to show a statistically reliable effect (\DIFdelbegin \DIFdel{21.8}\DIFdelend \DIFaddbegin \DIFadd{23.4}\DIFaddend \%).

\begin{table}

\caption{\label{tab:descriptives-table}Descriptive statistics of teams, acoustic analyses, and statistical analyses included in the meta-analysis. The data set included analyses from \DIFdelbeginFL \DIFdelFL{30 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{33 }\DIFaddendFL teams and  \DIFdelbeginFL \DIFdelFL{109 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{120 }\DIFaddendFL analysts.}
\centering
\DIFdelbeginFL %DIFDELCMD < \resizebox{\linewidth}{!}{
%DIFDELCMD < \begin{tabular}[t]{llrr}
%DIFDELCMD < \toprule
%DIFDELCMD < \bf{Team characteristics} &    & \bf{Range} & \bf{Median}\\
%DIFDELCMD < \midrule
%DIFDELCMD <  & Team size & 1.0 -- 12.0 & 3.0\\
%DIFDELCMD <  & Years after PhD & -3.8 -- 12.4 & 5.8\\
%DIFDELCMD <  & Prior belief & 48.5 -- 92.0 & 70.0\\
%DIFDELCMD <  & Acoustic analysis peer rating & 41.2 -- 88.3 & 75.2\\
%DIFDELCMD <  & Statistical analysis peer rating & 33.0 -- 93.3 & 74.1\\
%DIFDELCMD <  & Overall peer rating & 39.0 -- 88.7 & 72.2\\
%DIFDELCMD <  &  &  \vphantom{5} & \\
%DIFDELCMD < \bf{Acoustic analyses} &  & \bf{n} & \bf{\%}\\
%DIFDELCMD < \midrule
%DIFDELCMD < \hspace{1em}Outcome & F0 & 37 & 36\\
%DIFDELCMD <  & Duration & 36 & 35\\
%DIFDELCMD <  & Intensity & 15 & 14\\
%DIFDELCMD <  & Formants & 13 & 12\\
%DIFDELCMD <  & Other & 3 & 3\\
%DIFDELCMD <  &  &  \vphantom{4} & \\
%DIFDELCMD < \hspace{1em}Temporal window & Segment & 47 & 46\\
%DIFDELCMD <  & Word & 45 & 44\\
%DIFDELCMD <  & Sentence & 4 & 4\\
%DIFDELCMD <  & Phrase & 3 & 3\\
%DIFDELCMD <  & Other & 4 & 4\\
%DIFDELCMD <  &  &  \vphantom{3} & \\
%DIFDELCMD < \hspace{1em}Typicality operationalization & Categorical & 75 & 73\\
%DIFDELCMD <  & Continuous (mean) & 25 & 24\\
%DIFDELCMD <  & Continuous (median) & 3 & 3\\
%DIFDELCMD <  &  &  \vphantom{2} & \\
%DIFDELCMD < \bf{Statistical analyses} &  & \bf{n} & \bf{\%}\\
%DIFDELCMD < \midrule
%DIFDELCMD < \hspace{1em}Framework & Frequentist & 86 & 83\\
%DIFDELCMD <  & Bayesian & 18 & 17\\
%DIFDELCMD <  &  &  \vphantom{1} & \\
%DIFDELCMD < \hspace{1em}Model & Linear model & 102 & 98\\
%DIFDELCMD <  & GAM & 1 & 1\\
%DIFDELCMD <  & Other & 1 & 1\\
%DIFDELCMD <  &  &  & \\
%DIFDELCMD <  &  & \bf{Range} & \bf{Median}\\
%DIFDELCMD < \hspace{1em}N & Models & 1 -- 16 & 3\\
%DIFDELCMD <  & Predictors & 1 -- 5 & 2\\
%DIFDELCMD <  & Random terms & 1 -- 10 & 2\\
%DIFDELCMD < \hspace{1em} & \hspace{1em}Intercept & \hspace{1em}1 -- 10 & \hspace{1em}2\\
%DIFDELCMD < \hspace{1em} & \hspace{1em}Slope & \hspace{1em}0 -- 4 & \hspace{1em}0\\
...
The nested model yields a posterior 95\% CrI for between-team variability of 0 to \DIFdelbegin \DIFdel{0.05 }\DIFdelend \DIFaddbegin \DIFadd{0.04 }\DIFaddend standard units (\(\beta\) = 0.02, SD = 0.01), corresponding to a mean deviation range of about \(\pm 0\) to \(\pm 0.1\) standard units and 95\% probability.
The posterior 95\% CrI for between-analysis variability (nested within teams) is 0.11 to \DIFdelbegin \DIFdel{0.15 }\DIFdelend \DIFaddbegin \DIFadd{0.14 }\DIFaddend standard units (\(\beta\) = 0.132, SD = 0.01).
For the sake of illustration, these would correspond to an estimate of between-model variability in \DIFdelbegin \DIFdel{milliseconds (if looking for example at segment duration) }\DIFdelend \DIFaddbegin \DIFadd{segment and word durations }\DIFaddend that ranges between \DIFdelbegin \DIFdel{7.4 and 15.1 ms }\DIFdelend \DIFaddbegin \DIFadd{7 to 14 ms for segments and between 7 and 33 ms for words }\DIFaddend at 95\% credibility.
...
The meta-analytic model estimates the range of probable values of the standardized effect size to be between \DIFdelbegin \DIFdel{-0.024 and 0.021 }\DIFdelend \DIFaddbegin \DIFadd{-0.026 and 0.016 }\DIFaddend standard units (95\% CrI, mean = \DIFdelbegin \DIFdel{-0.001}\DIFdelend \DIFaddbegin \DIFadd{-0.005}\DIFaddend ).
In other words, our best guess is that speakers might not encode typicality in the acoustic signal (e.g.~by duration, f0, etc,) or, if they do, they do so by a maximum of \DIFdelbegin \DIFdel{\(\pm 0.02\) }\DIFdelend \DIFaddbegin \DIFadd{\(\pm 0.03\) }\DIFaddend standard units.
...
In this non-preregistered model, the meta-analytic intercept estimate is between \DIFdelbegin \DIFdel{-0.013 and 0.04 }\DIFdelend \DIFaddbegin \DIFadd{-0.016 and 0.03 }\DIFaddend standard units (95\% CrI, \(\beta\) = \DIFdelbegin \DIFdel{0.014}\DIFdelend \DIFaddbegin \DIFadd{0.008}\DIFaddend ).
This suggests that the acoustic measures of typical word combinations are \DIFdelbegin \DIFdel{0.01 }\DIFdelend \DIFaddbegin \DIFadd{0.02 }\DIFaddend standard units lower to \DIFdelbegin \DIFdel{0.04 }\DIFdelend \DIFaddbegin \DIFadd{0.03 }\DIFaddend standard units higher than the measures of atypical word combinations, at 95\% confidence.
\DIFdelbegin \DIFdel{Relative to the preregistered model, this model suggests a somewhat positive effect of typicality (although small negative effects are also possible)}\DIFdelend \DIFaddbegin \DIFadd{This result is qualitatively similar to the results obtained in the preregistered model}\DIFaddend .
...
Estimates ranged from \DIFdelbegin \DIFdel{-0.65 to 0.99 }\DIFdelend \DIFaddbegin \DIFadd{-0.7 to 1.01 }\DIFaddend standard units.

While the majority of model estimates and their uncertainty after shrinkage \DIFdelbegin \DIFdel{(in black) }\DIFdelend yields inconclusive results (i.e.~are compatible with a point null hypothesis), there are 27 model estimates for which the 95\% credible interval does not contain zero (\DIFdelbegin \DIFdel{in blue, 16}\DIFdelend \DIFaddbegin \DIFadd{14}\DIFaddend \%).
...
For example, for those analyses that investigated the duration of vowels (e.g.~the duration of the stressed vowel in \emph{\DIFdelbegin \DIFdel{Banane}\DIFdelend \DIFaddbegin \DIFadd{Banáne}\DIFaddend }), the reported duration measures exhibit standard deviations that range from 33.4 to 51.4 ms.
These standard deviations allow us to convert the \DIFdelbegin \DIFdel{meta analytic }\DIFdelend \DIFaddbegin \DIFadd{meta-analytic }\DIFaddend estimates into milliseconds by multiplying those values with the \DIFdelbegin \DIFdel{standard units}\DIFdelend \DIFaddbegin \DIFadd{the standard unit values of the meta-analytic estimates}\DIFaddend .
The reported effect estimates from teams varied between \DIFdelbegin \DIFdel{-0.65 and 0.99 }\DIFdelend \DIFaddbegin \DIFadd{-0.7 and 1.01 }\DIFaddend standard units, which corresponds to \DIFdelbegin \DIFdel{duration values ranging from -21.86 to 50.81 }\DIFdelend \DIFaddbegin \DIFadd{estimated segment duration differences (for atypical vs typical combinations) ranging from -23.34 to 33.84 }\DIFaddend ms.
A more conservative approach is to convert the meta-analytic estimates of between-model variation, thus obtaining an estimate of between-model variability in milliseconds that ranges between \DIFdelbegin \DIFdel{7.4 and 15.1 }\DIFdelend \DIFaddbegin \DIFadd{7.2 and 14.1 }\DIFaddend ms at 95\% credibility.
\DIFdelbegin \footnote{\DIFdel{The calculation is thus: the minimum standard deviation of duration multiplied by the lower limit of the 95\% CrI of the between-models variability estimate, times 1.96 to obtain a 95\% CrI: \(33.4 * 0.11 = 3.8\), \(3.8 * 1.96 = 7.4\) ms; the maximum standard deviation of duration multiplied by the upper limit of the 95\% CrI of the between-models variability estimate, times 1.96: \(51.4 * 0.15 = 7.7\), \(7.7 * 1.96 = 15.1\) ms.}}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{The calculation is thus: the minimum standard deviation of duration multiplied by the lower limit of the 95\% CrI of the between-models variability estimate, times 1.96 to obtain a 95\% CrI: 33.4 * 0.11 * 1.96 = 7.2 ms; the maximum standard deviation of duration multiplied by the upper limit of the 95\% CrI of the between-models variability estimate, times 1.96: 51.4 * 0.14 * 1.96 = 14.1 ms.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{Preregistration, i.e.~a time-stamped document in which researchers specify how they plan to collect their data and/or how they plan to conduct their confirmatory analysis, is can be a useful tool to safeguard researchers against the urge to explore many different analytical paths before choosing the one that, in hindsight, seems most justified.
...
\DIFdelbegin \DIFdel{In light of the observed analytic flexibility, there are a few things that researchers can do to appropriately calibrate confidence in their claims.
First of all, through }\DIFdelend \DIFaddbegin \DIFadd{Through }\DIFaddend sharing of materials, data and statistical protocols, we can make our idiosyncratic choices transparent to others \citep{munafo2017manifesto, vazire2017quality}.
...
\DIFdelbegin \DIFdel{Second, making }\DIFdelend \DIFaddbegin \DIFadd{Making }\DIFaddend analytic pathways completely re-traceable \DIFaddbegin \DIFadd{and preregistering them in advance }\DIFaddend does not change the fact that \DIFdelbegin \DIFdel{analysts }\DIFdelend \DIFaddbegin \DIFadd{different analysts might }\DIFaddend apply different analytic approaches \DIFaddbegin \DIFadd{(preregistered or not)}\DIFaddend .
...
\DIFdelbegin \DIFdel{Third, if }\DIFdelend \DIFaddbegin \DIFadd{If }\DIFaddend we develop a good understanding of relevant analytic degrees of freedom, we could apply all conceivable analytic strategies and compare the results across all combinations of these choices.
...
Possible tools to strengthen theoretical development \DIFdelbegin \DIFdel{are }\DIFdelend relate to mathematically formalizing verbal expectations or using computational models \citep[e.g.,][]{van2020formalizing, guest2021computational, scheel2021hypothesis, devezer2021case}.
\DIFaddbegin \DIFadd{Although conceptually promising, in their current state, such formalized models typically work in spaces that are much lower in dimensionality than the complex systems in which we measure. Thus, future research should spend resources on attempting to quantitatively relate the abstract theoretical space to the complex measurement space.
}\DIFaddend 
...
See \DIFdelbegin %DIFDELCMD < \url{https://github.com/many-speech-analyses/many_analyses/blob/main/figs/credit-taxonomy.png}%%%
\DIFdelend \DIFaddbegin \url{https://github.com/many-speech-analyses/many_analyses/blob/main/figs/credit-taxonomy-all.png}\DIFaddend .

\hypertarget{conflicts-of-interest}{%
\section{Conflicts of interest}\label{conflicts-of-interest}}

We have no conflicts of interest to disclose\DIFaddbegin \DIFadd{.
}

\hypertarget{acknowledgements}{%
\section{Acknowledgements}\label{acknowledgements}}

\DIFadd{We would like to thank the editors David Sbarra and Julia Strand, Matthew Goldrick and two anonymous reviewers for their insightful comments and suggestions.
}

\DIFadd{The initiating authors (SC, JVC, TR) wish to express immense gratitude to all the other authors, without which this project would not have come to be.
}

\DIFadd{All remaining errors are the initiating authors' (SC, JVC, TR)
}

\hypertarget{funding}{%
\section{Funding}\label{funding}}

\DIFadd{Stefano Coretta was partially supported by the European Research Council Grant No.~742289 Human interaction and the evolution of spoken accent (2017--2022) awarded to Prof.~Jonathan Harrington.
}

\DIFadd{Jalal Al-Tamimi was partially supported by the French Investissements d'Avenir - Labex EFL program (ANR-10- LABX-0083), contributing to the IdEx Université Paris Cité - ANR-18-IDEX-0001.
}

\DIFadd{George Moroz, Sita Carraturo: The research leading to these results has received funding from the Basic Research Program at the National Research University Higher School of Economics.
}

\DIFadd{Iñigo Urrestarazu-Porta: The research leading to these results has received funding from the MADPAB (National Research Agency of the French republic, ANR-20-CE27-0007) and Monumenta Linguae Vasconum 6: avances en la cronología de la historia y la prehistoria de la lengua vasca projects (Ministry of Science and Innovation of the Spanish kingdom, PID2020-118445GB-I00).
}

\DIFadd{Constantijn Kaland, Martine Grice: German Research Foundation (DFG) - Project-ID 281511265 - SFB 125.
}

\DIFadd{Nina R. Benway: This research was supported in part through computational resources provided by Syracuse University (NSF ACI-1341006; NSF ACI-1541396)}\DIFaddend .
