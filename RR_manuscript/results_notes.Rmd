...
#
# NOTE: This can be simplified using 03_analyses_coding.Rmd
# I'll come back to this later and rewrite
#
team_leaders <- readr::read_csv(
  ...
  mutate(leader = Q6, 
    leader = case_when(
    Q6 %in% c("Ali A-Hoorie", "Ali Al-Hoorie", "Ali H. Al-Hoorie") ~ "Ali Al-Hoorie", 
    Q6 %in% c("Alicea Megan Brown", "Alicia Brown") ~ "Alicia Brown", 
    Q6 %in% c("Dr Rory Turnbull", "Rory Turnbull") ~ "Rory Turnbull", 
    Q6 %in% c("Dr. Reenu Punnoose", "Reenu Punnoose") ~ "Reenu Punnoose", 
    Q6 %in% c("Dušan Nickolić", "Dušan Nikolić", "Dusan Nikolic", "Dusan Nikolic (dusan.nikolic1@ucalgary.ca)") ~ "Dusan Nikolic", 
    ...
    Q6 %in% c("Scott James Perru", "Scott Perry") ~ "Scott James Perru", 
    ...
    Q6 %in% c("Vincent Hughes", "Vince Hughes") ~ "Vincent Hughes", 
    TRUE ~ leader
  )) %>% 
...
# Need to join team leaders with msa_models$team to get the number of members on 
# each team (unless this data already lives somewhere else) 
# We could use : 
# https://docs.google.com/spreadsheets/d/1w39rU7O5ijzPmCpClVt5LEDIhzaLBDNrwfdMJUP86q8/edit#gid=0 
# But I'm thinking we need to make anonymous the team leader name info (offline) 
# for any of this to be a permanent solution. 
...
`r team_desc$n_teams %>% xfun::numbers_to_words() %>% stringr::str_to_title()` teams initially signed up to participate. 
`r (team_desc$n_teams - no_teams) %>% xfun::numbers_to_words() %>% stringr::str_to_title()` of the proposed teams dropped out of the project during the analysis phase^[ADD REASONS]. 
...
<!-- 
JVC: I pulled the initial total from the uptake doc (see line 58-n) 
I think this is the most accurate count, by Stef might have another way
--> 
...
analytic approaches widely varied along the following dimensions:
...
As shown by the results of this meta-anlyitic study, such complex process has led to a wide "garden of forking paths", i.e. to a wide range of combinations of possible analitic decisions.
Each individual analyst team has chosen unique paths to acoustically measure, operationalize and statistical analyse the data.
...
These findings are very much in line with previous crowd-sourced projects that suggest variation between teams is neither driven by perceived quality of the analysis nor by analysts biases or experience (REF,REF,REF).
Given the mounting evidence, Breznau et al. (2021) conclude that "[...] we are left to believe that idiosyncratic uncertainty is a fundamental feature of the scientific process that is not easily explained by typically observed researcher characteristics or analytic decisions."
...
While we agree with the importance of direct replications, our study (and similar crowd-sourced analyses before us) suggest that replicating more is not enough.
...
A desired outcome of knowomg that different but reasonable measurement choices or statistical approaches might lead to entirely different interpretations of research data is to calibrate our (un)certainty in the strength of the collected evidence and, in turn, communicate that (un)certainty appropriately.
...
Moreover, while the number of participating teams is larger than most earlier crowdsourcing projects, it is likely too small to estimate meta-analytic estimates reliably.
