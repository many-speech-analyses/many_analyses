---
title: |
  Multidimensional signals and analytic flexibility:  
...
```{r}
#| label: sankey-plot
#| fig.cap: "Sankey plot illustrating workflow choices from analyst teams."
#| out.width: "100%"
knitr::include_graphics("../figs/flow_outcome.png", error = FALSE)
```
...
msa_models_unique <- left_join(
  ...
  readRDS("./data/peer_review/msa_reviews.rds") %>% 
    ...
    summarize(phon_rating = median(phon_rating), 
              stat_rating = median(stat_rating), 
              all_rating = median(all_rating)), 
  by = "animal"
) %>% 
  left_join(., 
...
team_leaders <- readr::read_csv("./data/questionnaires/msa_intake_form.csv") %>%
...
submitted_animals <- readRDS("./data/analyses/msa_models.rds") %>% 
  pull(animal) %>% 
  unique
...
tbl_team_desc <- msa_models_unique %>% 
  ...
  add_row(.before = 1, 
    ...
    Median = team_desc_df %>% filter(stage == "Submitted") %>% pull(median_size), 
...
mean_analyses <- msa_models_unique %>% 
  ...
  summarise(
    ...
    median_models = median(max_n_models, na.rm = T), 
...
predictor_n <- msa_models_unique %>% 
  summarise(
    ...
    median_predictor = median(n_predictors, na.rm = T), 
...
bind_rows(
  ...
  add_row(`\\bf{Team characteristics}` = "\\bf{Statistical analyses}", `  ` = " ", 
    `\\bf{Range}` = "\\bf{n}", `\\bf{Median}` = "\\bf{\\%}", .before = 29) %>%
  add_row(`\\bf{Team characteristics}` = " ", `  ` = " ", `\\bf{Range}` = " ",
    `\\bf{Median}` = " ", .before = 33) %>%
  add_row(`\\bf{Team characteristics}` = " ", `  ` = " ", `\\bf{Range}` = " ", 
    `\\bf{Median}` = " ", .before = 38) %>%
  ...
  kbl(., booktabs = T, align = c("l", "l", "r", "r"), escape = F, 
    ...
    label = "descriptives-table") %>% 
  kable_styling(full_width = F, font_size = 8, latex_options = "scale_down") %>% 
