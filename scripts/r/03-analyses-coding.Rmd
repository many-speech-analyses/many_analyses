...
This is just a copy/paste of what I had in the other rmd file. 
It still needs to be cleaned up (there is probably overlap with the code above in terms of packages loaded, etc.). 
...
msa_models <- dir_ls(path = here("models"), regexp = ".rds$") %>% 
  as_tibble() %>% 
  transmute(path = value, 
    mod_name = str_remove(path, here("models/")), 
