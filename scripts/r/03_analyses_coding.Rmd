...
clean_up <- . %>% 
  tidy(effects = "fixed", parametric = TRUE) %>% 
  suppressWarnings() %>% 
...
msa_models <- dir_ls(path = here("data/analyses/models"), regexp = ".rds$") %>% 
  ...
  # because it warns every time that some coef have underscores and its annoying
  suppressWarnings() %>%
  select(-model)
...
msa_models %>% 
  unnest(sum) %>%
  filter(mod_name != "polymetme_brevirostrum_1_dur_cat") %>%
  mutate(
  ...
  geom_segment(aes(x = estimate - se * 1.96, xend = estimate + se * 1.96, yend = mod_name), 
    color = "#cc0033", alpha = 0.5, size = 3, lineend = "round") + 
  geom_point(color = "#cc0033", pch = 21, stroke = 3, size = 3.1) +
  facet_wrap(typicality_lab ~ ., ncol = 2, scales = "free") +
