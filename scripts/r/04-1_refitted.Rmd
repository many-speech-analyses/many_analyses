...
~~Here's the code that does that for you.~~ The code has repeatedly failed to upload ALL models. Please upload manually.
...
The code has repeatedly failed to download ALL models. Please download manually.
...
For the following to run, it is necessary to have the individual refitted models in `data/analyses/models`.

```{r get-divergent-transitions}
...
divergent_transitions <- 
  ...
  filter(mod_name != "petauroides_fistulator_1_f0_cat") %>% 
  mutate(
