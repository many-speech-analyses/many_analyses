...
pervagor_meeki_2_durnoun_cat <- brm(
  Duration_ms_s ~ 
         effect_cat + 
         + (1| speaker),
  seed = 111,
  data = data_dur_trim[data_dur_trim$word_type == "target_name",], 
  cores = 4,  
  file = "./data/analyses/models/pervagor_meeki_2_durnoun_cat"
  )

pervagor_meeki_3_f0meannoun_cat <- brm(
  Mean_f0_st_s ~ 
         effect_cat + 
         + (effect_cat | speaker),
  seed = 111,
  data = data_meanf0_trim[data_meanf0_trim$word_type == "target_name",], 
  cores = 4,  
  file = "./data/analyses/models/pervagor_meeki_3_f0meannoun_cat"
  )

pervagor_meeki_4_f0maxnoun_cat <- brm(
  Max_f0_st_s ~ 
         effect_cat + 
         + (1| speaker),
  seed = 111,
  data = data_maxf0_trim[data_maxf0_trim$word_type == "target_name",], 
  cores = 4,  
  file = "./data/analyses/models/pervagor_meeki_4_f0maxnoun_cat"
  )
...
pervagor_meeki_6_duradj_cat <- brm(
  Duration_ms_s ~ 
         effect_cat + 
          + (1 | speaker)
          + (effect_cat | target_word),
  ...
  data = data_dur_trim[data_dur_trim$word_type == "target_colour",], 
  cores = 4,  
  file = "./data/analyses/models/pervagor_meeki_6_duradj_cat"
  )

pervagor_meeki_7_f0meanadj_cat <- brm(
  Mean_f0_st_s ~ 
         effect_cat + 
         + (1 | speaker)
         + (1 | target_word),
  ...
  data = data_meanf0_trim[data_meanf0_trim$word_type == "target_colour",], 
  cores = 4,  
  file = "./data/analyses/models/pervagor_meeki_7_f0meanadj_cat"
  )

pervagor_meeki_8_f0maxadj_cat <- brm(
  Max_f0_st_s ~ 
         effect_cat + 
         + (1 | speaker)
         + (1 | target_word),
  ...
  data = data_maxf0_trim[data_maxf0_trim$word_type == "target_colour",], 
  cores = 4,  
  file = "./data/analyses/models/pervagor_meeki_8_f0maxadj_cat"
  )
...
data <- read.delim("./data/analyses/tr/procambarus_maculosus/selectedData.txt", header = T, sep = "") %>% 
  as_tibble() # %>% 
...
info <- data %>% 
  dplyr::select(sl_rowIdx, bundle, vowel, speaker, sex, typicality, word) %>% 
  unique()
...
basis <- fda::create.bspline.basis(rangeval = c(0, 1), nbasis = nBasis, norder = nOrder)

# define functional parameter object (will be used to estimate functional parameters later)
fdParObj <- fda::fdPar(fdobj = basis, Lfdobj = Lfdobj, lambda = lambda)

coef <- data %>%
  group_by(sl_rowIdx) %>%
  summarise(coef = c(fda::smooth.basis(argvals = times_norm, y = F0, fdParObj)$fd$coefs),
...
y_fd <- fda::fd(coef = coef, basisobj = basis)

# this is the actual FPCA
nPCs <- 3
y_pcafd <- fda::pca.fd(y_fd, nharm = nPCs, fdParObj)
...
```{r procambarus_maculosus_models}

s1.lmer <- lmer(s1 ~ typicality * sex + (sex | word) + (typicality | speaker), data = info,
                control = lmerControl(optimizer = "optimx", optCtrl = list(method = "nlminb"))) %>% 
  step() %>% get_model()

# they did not interpret the models with typicality in there, so the minimum model we refit will be typicality + sex

s2.lmer <- lmer(s2 ~ typicality * sex + (sex | word) + (typicality | speaker), data = info,
                control = lmerControl(optimizer = "optimx", optCtrl = list(method = "nlminb"))) %>% 
  step() %>% get_model()
# s2 ~ typicality + (typicality | speaker), but failed to converge

s2.lmer <- lmer(s2 ~ typicality + (1 | word) + (1 | speaker), data = info,
                control = lmerControl(optimizer = "optimx", optCtrl = list(method = "nlminb"))) 
s2.lmer %>% summary(corr = F)


```

```{r procambarus_maculosus_refitting}

dat <- info %>% 
  mutate(
         # scale continuous variables
         s1_s = (s1 - mean(s1, na.rm = TRUE)) / sd(s1,  na.rm = TRUE),
         s2_s = (s2 - mean(s2, na.rm = TRUE)) / sd(s2,  na.rm = TRUE),
         sex_s = as.factor(sex),
         sex_s = C(sex_s, sum),
         effect_cat = as.factor(typicality),
         effect_cat = C(effect_cat, treatment)
...
procambarus_maculosus_1_pca1_cat <- brm(
  s1_s ~ 
         effect_cat + sex_s + 
    (sex_s | word) + 
    (effect_cat | speaker),
  ...
  data = dat, 
  cores = 4,  
  file = "./data/analyses/models/procambarus_maculosus_1_pca1_cat"
  )

# run brms on standardized models
procambarus_maculosus_2_pca2_cat <- brm(
  s2_s ~ 
         effect_cat + 
    (1 | word) +
    (1 | speaker),
  ...
  data = dat, 
  cores = 4,  
  file = "./data/analyses/models/procambarus_maculosus_2_pca2_cat"
  )
...
```{r pseudopleuronectes_assasi_setup}

# read csv
mpa <- read.delim("./data/analyses/tr/pseudopleuronectes_assasi/alldata.txt", header = T, sep = "") %>% 
  as_tibble() 

atypical <- subset(mpa,Typicality=="atypical")
atypicalAF <- subset(atypical,Condition=="AF")
atypicalNF <- subset(atypical,Condition=="NF")
atypicalAF1 <- subset(atypicalAF,Vowel==1)
atypicalAF2 <- subset(atypicalAF,Vowel==2)
atypicalNF1 <- subset(atypicalNF,Vowel==1)
atypicalNF2 <- subset(atypicalNF,Vowel==2)
atypical1 <- subset(atypical,Vowel==1)
atypical2 <- subset(atypical,Vowel==2)
atypicalAFNF1 <- rbind(atypicalAF1,atypicalNF1)
NF <- subset(mpa,Condition=="NF")
NFa <- subset(NF,Typicality=="atypical")
NFm <- subset(NF,Typicality=="medium")
NFt <- subset(NF,Typicality=="typical")
NF1 <- subset(NF,Vowel==1)
NF2 <- subset(NF,Vowel==2)
```

```{r pseudopleuronectes_assasi_models}

NF1DurTyp <- lmer(Duration~Typicality+(1|Speaker)+(1|TargetColor),data=NF1,REML=FALSE)
NF1F0Typ <- lmer(MedianST~Sex+Typicality+(1|Speaker)+(1|TargetColor),data=NF1,REML=FALSE)
NF1IntTyp <- lmer(MedianInt~Typicality+(1|Speaker)+(1|TargetColor),data=NF1,REML=FALSE)
NF1DistTyp <- lmer(FormDist~Typicality+(1|Speaker)+(1|TargetColor),data=NF1,REML=FALSE)

```

```{r pseudopleuronectes_assasi_refitting}

# standardize
NF1 <- NF1 %>% 
    mutate(
    Duration_s = (Duration - mean(Duration, na.rm = TRUE)) / sd(Duration,  na.rm = TRUE),
    MedianST_s = (MedianST - mean(MedianST, na.rm = TRUE)) / sd(MedianST,  na.rm = TRUE),
    MedianInt_s = (MedianInt - mean(MedianInt, na.rm = TRUE)) / sd(MedianInt,  na.rm = TRUE),
    FormDist_s = (FormDist - mean(FormDist, na.rm = TRUE)) / sd(FormDist,  na.rm = TRUE),
    effect_cat = C(as.factor(Typicality), treatment),
    sex_s = as.factor(Sex),
    sex_s = C(sex_s, sum)
  )
    
pseudopleuronectes_assasi_1_dur_cat <- brm(
  Duration_s ~ 
         effect_cat + 
         (1|Speaker) + 
         (1|TargetColor),
  ...
  data = NF1, 
  cores = 4,  
  file = "./data/analyses/models/pseudopleuronectes_assasi_1_dur_cat"
  )

pseudopleuronectes_assasi_2_f0_cat <- brm(
  MedianST ~ 
         effect_cat + sex_s +
         (1|Speaker) + 
         (1|TargetColor),
  ...
  data = NF1, 
  cores = 4,  
  file = "./data/analyses/models/pseudopleuronectes_assasi_2_f0_cat"
  )

pseudopleuronectes_assasi_3_int_cat <- brm(
  FormDist_s ~ 
         effect_cat + 
         (1|Speaker) + 
         (1|TargetColor),
  ...
  data = NF1, 
  cores = 4,  
  file = "./data/analyses/models/pseudopleuronectes_assasi_3_int_cat"
  )

pseudopleuronectes_assasi_4_formants_cat <- brm(
  MedianInt_s ~ 
         effect_cat + 
         (1|Speaker) + 
         (1|TargetColor),
  ...
  data = NF1, 
  cores = 4,  
  file = "./data/analyses/models/pseudopleuronectes_assasi_4_formants_cat"
  )
