...
predictors_bm_rintercepts <- brm(
  ...
  threads = threading(2),
  backend = "cmdstanr",
  file = "./data/meta_analysis/predictors_bm_rintercepts_only_est.rds"
)
...
```{r}
print(predictors_bm_rintercepts_me, digits = 4)

# notable for paper:
# the more models in analysis, the smaller the more negative the effect. note: not really interpretable I think
# both outcome and window, two prime dfs for phonetic analysis, seems to affect the outcome. Useful to make raw-data plot with model estimate overlaid
...
post_pred <- predictors_bm_rintercepts_me %>%
  ...
  mutate(predictor = fct_recode(predictor, 
                                ...
                                "years_from_phd" = "b_years_from_phd_s",
                                "model uniqueness" = "b_pop_sdi_s",
                                "model conservativeness"  = "b_n_random_s",
...
# order predictors
post_pred_forest$predictor <- factor(post_pred_forest$predictor, levels = c(# team-level predictors
                                                                            "years after Phd",
                                                                            "team's number of models",
                                                                            "prior belief",
                                                                            "peer rating", 
...
## extract
post_conservatives <- predictors_bm_rintercepts %>%
  spread_draws(b_Intercept, b_n_random_s) %>%
  ## make a list of relevant value range of b_years_from_phd
  mutate(random = list(seq(min(msa_models$n_random_s), max(msa_models$n_random_s), 0.1)))%>%
  unnest(random) %>%
  mutate(prediction = b_Intercept + (b_n_random_s * random)) %>%
  group_by(random) %>%
...
msa_agg_conservative <- msa_models %>% 
  group_by(animal) %>% 
  summarise(estimate_m = mean(estimate),
            se_m = mean(se),
            meas_error = estimate_m / se_m,
            random_m = mean(n_random_s),
            number = n())

# look at model conservatives
ggplot(post_conservatives,
       aes(x = random,
           y = pred_mean)) +
  # model SE
  geom_ribbon(aes(ymin = l95, 
                  ymax = h95),
              color = "grey",
              alpha = 0.3) +
  # model prediction
  geom_line(size = 2) +
  # raw data grouped by team and sized by number of models
  geom_point(data = msa_agg_conservative,
             aes(x = random_m,
                 y = meas_error,
                 size = number,
                 color = animal
             ...
             alpha = 0.5) +
  # linear smooth of raw data
  geom_smooth(data = msa_agg_conservative,
              method = "lm",
             aes(x = random_m,
                 y = meas_error,
                 size = number,
...
# looks like n_models has the highest vif (~3), which is moderate (Zuur, 2010)
...
# ricardo fusarolli
#"Could you plot the raw outcome measure w uncertainty, the mean without me and the mean of the model w me? No predictors for now"

ggplot(msa_models,
       aes(x = estimate,
           y = reorder(model_id, estimate))
       ) +
  geom_point(size = 0.5) +
  geom_errorbar(aes(xmin = estimate - se,
                    xmax = estimate + se),
                 width = 0,
                alpha = 0.45) +
  geom_vline(xintercept = mean(msa_models$estimate),
             color = "darkred",
             size = 1) +
  geom_vline( xintercept = mean(post_pred_forest[post_pred_forest$predictor == "Intercept",]$post_mean),
             color = "blue",
             lty = "dashed",
             size = 1) + 
  labs(y = "") +
