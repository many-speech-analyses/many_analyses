...
1. Get a list of the subset of teams that need to be downloaded (Max = 30 because the final model only includes 30 teams). 
...
Of the 30 teams included in the meta-analysis, `r length(final_team_names)` included either F0, duration or intensity in the analyses. 
...
If we can't source the lines directly without touching the refit_xx.Rmd files (because *some* people refuse to use `here`), then we will have to copy/paste code to this file and adjusts the paths like heathens. 
...
anomalocaris_ornata_df <- tribble(
  ~"team", ~"model_n", ~"dv", ~"mean", ~"sd", 
  "anomalocaris_ornata", 1, "intensity", mean(df.int$Mean_dB^2),        sd(df.int$Mean_dB^2), 
...
naso_cassivellaunos_df <- tribble(
  ~"team", ~"model_n", ~"dv", ~"mean", ~"sd", 
  "naso_cassivellaunos", 1, "f0", mean(msa$DeltaF0_rel), sd(msa$DeltaF0_rel), 
  "naso_cassivellaunos", 2, "f0", mean(msa$sync_rel), sd(msa$sync_rel),
)
...
trapezia_cantonensis_df <- tribble(
  ~"team", ~"model_n", ~"dv", ~"mean", ~"sd", 
  "trapezia_cantonensis", 1, "duration", mean(MSA_trapezia_cantonensis_data$dur), sd(MSA_trapezia_cantonensis_data$dur), 
)
...
# This part won't work because some coders refuse to use `here`
# source_part(
...
eriphia_laterispinis_df <- tribble(
  ...
  "eriphia_laterispinis", 2, "intensity", mean(msa_data$mean_int), sd(msa_data$mean_int), 
  "eriphia_laterispinis", 3, "duration",  mean(msa_data$duration), sd(msa_data$duration), 
)
...
nestor_idahoensis_df <- tribble(
  ...
  "nestor_idahoensis", 2, "f0",       mean(msa_data$f0), sd(msa_data$f0), 
)
...
# Super long because somebody refuses to use `here` and I don't want to modify `refit_sc.Rmd`
