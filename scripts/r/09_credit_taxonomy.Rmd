...
There were `r filter(contributors, is.na(email_address)) %>% nrow` individuals that didn't include their email (`r filter(contributors, is.na(email_address)) %>% pull(name) %>% knitr::combine_words(words = .)`). 
