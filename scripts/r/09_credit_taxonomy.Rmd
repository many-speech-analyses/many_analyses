...
`r filter(affiliations, is.na(primary_affiliation_university)) %>% pull(n) %>% xfun::numbers_to_words() %>% str_to_sentence()` analysts did not include their primary affiliation. 
...
cr_prep <- contributors %>% 
  select(name, surname, conceptualization:writing_review_editing, is_initiating, initials) %>% 
  pivot_longer(
...
contributors %>% 
  select(name, primary_affiliation_university) %>% 
  transmute(
    name, 
...
contributors %>% 
  select(name, primary_affiliation_university, primary_affiliation_department) %>% 
  transmute(
    name, 
    num_man = glue::glue("- num: {1:nrow(.) + 1}"), 
    print = glue::glue("- org: {primary_affiliation_department}, {primary_affiliation_university}"), 
  ) %>% 
  pivot_longer(cols = -name, names_to = "col", values_to = "print") %>% 
  pull(print) %>% 
  clipr::write_clip() 
