...
initiating_names <- c("Stefano Coretta", "Joseph V. Casillas", 
                      "Simon Roessig", "Michael Franke", "Timo B. Roettger")

contributor_roles <- rev(c(
...
cr_prep <- contributors %>% 
  select(name:writing_review_editing) %>% 
  pivot_longer(cols = -name, names_to = "contributor_roles", values_to = "val") %>% 
  ...
  mutate(initials = gsub('\\b(\\pL)\\pL{2,}|.','\\U\\1', name, perl = TRUE), 
    is_initiating = if_else(name %in% initiating_names, 1, 0), 
...
cr_prep %>% 
  ...
  aes(x = name, y = contributor_roles, alpha = val) +
    ...
    scale_y_discrete(labels = contributor_roles) + 
...
cr_prep %>% 
  ...
  aes(y = initials, x = contributor_roles, alpha = val) +
    geom_point(size = 2, pch = 18, show.legend = F) +
    scale_x_discrete(position = "top", labels = rev(contributor_roles)) + 
    scale_alpha_continuous(range = c(0.05, 1)) +
    labs(x = "Contributor roles", y = "Analysts",
         caption = "Author contributions based on CRediT taxonomy") +
    theme_minimal(base_size = 12, base_family = "Times") +
    theme(
      axis.title.y = element_text(size = rel(.9), hjust = 0.95),
      ...
      panel.grid.major = element_line(colour = 'grey90', size = 0.15),
      panel.grid.minor = element_line(colour = 'grey90', size = 0.15), 
      axis.text.x.top = element_text(angle = 45, hjust = 0, size = rel(0.6)), 
