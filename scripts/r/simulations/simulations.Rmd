...
```{r include=FALSE}
dir.create("./models/sim/", F, recursive = T)
```
...
Prior predictive checks are run to assess the goodness of the prior specification before model fitting.
...
Let's plot the prior probability distributions of the intercept and the standard deviation.
...
# Analytic and researchers' predictors model
...
\mu_{\alpha_{t[i]}}       & = \beta + \upsilon_u\cdot uniq_i + \upsilon_c\cdot cons_i + \upsilon_p\cdot phoc_i + \upsilon_m\cdot modn_i \\
 & \qquad + \upsilon_d\cdot mdim_i + \upsilon_w\cdot twin_i \\
